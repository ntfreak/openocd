...
void jtag_add_bitbang(uint8_t values, uint8_t mask)
{
	interface_add_bitbang(values, mask);
}
...
int jtag_init(struct command_context *cmd_ctx)
{
	...
	if (jtag_reset_config & RESET_CNCT_TCK_LOW_RELEASE_SRST) {
		if (jtag_reset_config & RESET_HAS_SRST) {
			jtag_add_bitbang(0x00, JTAG_PIN_SRST | JTAG_PIN_TCK);
			jtag_add_sleep(10000);
			jtag_add_bitbang(0xff, JTAG_PIN_SRST);
			jtag_add_sleep(10000);
		} else {
