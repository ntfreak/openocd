...
static int xlnx_pcie_xvc_init(void)
{
	...
	LOG_INFO("Scanning device %s's extended capabilities for XVC/PCIe ...", xlnx_pcie_xvc->device);
	/* Parse the PCIe extended capability list and try to find vendor specific header */
	...
	while (xlnx_pcie_xvc->offset <= PCI_CFG_SPACE_EXP_SIZE - 4 &&
		...
		LOG_DEBUG("Checking capability at 0x%x; id=%04x version=%x next=%x",
		...
		if (PCI_EXT_CAP_ID(cap) == PCI_EXT_CAP_ID_VNDR) {
			...
			LOG_DEBUG("Checking possible match at %x; id: %x; rev: %x; length: %x",
				 xlnx_pcie_xvc->offset,
