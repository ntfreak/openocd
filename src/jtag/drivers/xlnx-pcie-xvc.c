...
static int xlnx_pcie_xvc_swd_line_reset(void)
{
	int err;

	err = xlnx_pcie_xvc_transact(32, 0xffffffff, 0, NULL);
	if (err != ERROR_OK)
		return err;
	err = xlnx_pcie_xvc_transact(32, 0xffffffff, 0, NULL);
	if (err != ERROR_OK)
...
static int xlnx_pcie_xvc_swd_switch_seq(enum swd_special_seq seq)
{
	...
	case LINE_RESET:
		...
		return xlnx_pcie_xvc_swd_line_reset();
	case JTAG_TO_SWD:
		LOG_DEBUG("JTAG-to-SWD");
		err = xlnx_pcie_xvc_swd_line_reset();
		if (err != ERROR_OK)
			return err;
		err = xlnx_pcie_xvc_transact(16, 0xe79e, 0, NULL);
		if (err != ERROR_OK)
			return err;
		err = xlnx_pcie_xvc_swd_line_reset();
		if (err != ERROR_OK)
			return err;
		return xlnx_pcie_xvc_transact(2, 0, 0, NULL);
	case SWD_TO_JTAG:
		LOG_DEBUG("SWD-to-JTAG");
		err = xlnx_pcie_xvc_swd_line_reset();
		if (err != ERROR_OK)
			return err;
		err = xlnx_pcie_xvc_transact(16, 0xe73c, 0, NULL);
		if (err != ERROR_OK)
			return err;
		return xlnx_pcie_xvc_swd_line_reset();
	default:
