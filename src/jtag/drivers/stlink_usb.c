...
static int stlink_tcp_open(void *handle, struct hl_interface_param_s *param)
{
	...
	res = setsockopt(h->tcp_backend_priv.fd, IPPROTO_TCP, TCP_NODELAY, &tcp_nodelay_optval, sizeof(int));
	if (res == -1) {
	...
	res = setsockopt(h->tcp_backend_priv.fd, SOL_SOCKET, SO_RCVBUF, &so_recv_optval, sizeof(int));
	if (res == -1) {
	...
	res = setsockopt(h->tcp_backend_priv.fd, SOL_SOCKET, SO_SNDBUF, &so_sndbuf_optval, sizeof(int));
	if (res == -1) {
