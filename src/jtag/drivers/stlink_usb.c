								   ...
								   uint32_t count, uint8_t *buffer)
{
	...
	LOG_DEBUG("read at 0x%08x len %d*0x%08x", addr, size, count);
	count *= size;
									...
									uint32_t count, const uint8_t *buffer)
{
	...
	LOG_DEBUG("write at 0x%08x len %d*0x%08x", addr, size, count);
	count *= size;
...
COMMAND_HANDLER(stlink_dap_use_stlink_server_command)
{
	...
	stlink_dap_param.stlink_server_port = 7184; /* default stlink-server port */
