...
static int stlink_tcp_send_cmd(void *handle, int send_size, int recv_size, bool check_tcp_status)
{
	...
	int sent_size = send(h->tcp_backend_priv.fd, (char *) h->tcp_backend_priv.send_buf, send_size, 0);
	...
	int received_size = recv(h->tcp_backend_priv.fd, (char *) h->tcp_backend_priv.recv_buf, recv_size, 0);
