...
{
	struct stlink_usb_handle_s *h = handle;
	return h->backend->bulk_read(handle, buf, size);
}
...
{
	struct stlink_usb_handle_s *h = handle;
	struct stlink_usb_priv_s *usb_priv = h->usb_backend_priv;
	int tr, ret;
...
{
	struct stlink_usb_handle_s *h = handle;
	struct stlink_tcp_priv_s *tcp_priv = h->tcp_backend_priv;

	assert(handle != NULL);

	/* send the TCP command */
	int sent_size = send(tcp_priv->fd, (char *) cmd, cmd_size, 0);
	...
	int received_size = recv(tcp_priv->fd, (char *) data, data_size, 0);
...
static int stlink_tcp_xfer_noerrcheck(void *handle, const uint8_t *buf, int size)
{
	...
	h_u32_to_le(&cmdbuf[4], tcp_priv->connect_id);
...
static int stlink_tcp_close(void *handle)
{
	...
	if (h && tcp_priv->connected) {
		if (tcp_priv->connect_id) {
			stlink_usb_exit_mode(h);
...
static int stlink_usb_usb_open(void *handle, struct hl_interface_param_s *param)
{
	...
	h->usb_backend_priv = calloc(1, sizeof(struct stlink_usb_priv_s));
	if (h->usb_backend_priv == NULL)
		return ERROR_FAIL;
...
static int stlink_tcp_open(void *handle, struct hl_interface_param_s *param)
{
	...
	h->tcp_backend_priv = calloc(1, sizeof(struct stlink_tcp_priv_s));
	if (h->tcp_backend_priv == NULL)
		return ERROR_FAIL;
	...
	tcp_priv->fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	tcp_priv->connected = false;
	tcp_priv->device_id = 0;
	tcp_priv->connect_id = 0;
	...
	LOG_DEBUG("socket : %x", tcp_priv->fd);

	int res;

	int flag = 1;
	res = setsockopt(tcp_priv->fd, IPPROTO_TCP, TCP_NODELAY, (char *) &flag, sizeof(flag));
	if (res == -1) {
	...
	int a = 49152;
	res = setsockopt(tcp_priv->fd, SOL_SOCKET, SO_RCVBUF, (char *) &a, sizeof(a));
	if (res == -1) {
		LOG_ERROR("cannot set sock option 'SO_RCVBUF', errno: %s", strerror(errno));
		return ERROR_FAIL;
	}

	res = setsockopt(tcp_priv->fd, SOL_SOCKET, SO_SNDBUF, (char *) &a, sizeof(a));
	if (res == -1) {
		LOG_ERROR("cannot set sock option 'SO_SNDBUF', errno: %s", strerror(errno));
		return ERROR_FAIL;
	}

	if (connect(tcp_priv->fd, (const struct sockaddr *) &serv, sizeof(serv)) == -1) {
		LOG_ERROR("cannot connect to stlink server");
		return ERROR_FAIL;
	}

	tcp_priv->connected = true;
	...
	for (uint32_t stlink_id = 0; stlink_id < connected_stlinks; stlink_id++) {
		...
		tcp_priv->device_id = le_to_h_u32(&databuf[4]);
	...
	memset(&cmdbuf[1], 0, 4); /* reserved */
	h_u32_to_le(&cmdbuf[4], tcp_priv->device_id);
	ret = stlink_tcp_send_cmd(h, cmdbuf, 8, databuf, 8, true);
	if (ret != ERROR_OK)
		return ret;

	tcp_priv->connect_id = le_to_h_u32(&databuf[4]);
...
static struct stlink_backend_s stlink_usb_itf = {
...
static struct stlink_backend_s stlink_tcp_itf = {
...
static int stlink_open(struct hl_interface_param_s *param, enum stlink_mode mode, void **fd)
{
	...
	if (param->use_stlink_tcp)
		h->backend = &stlink_tcp_itf;
	else
		h->backend = &stlink_usb_itf;
