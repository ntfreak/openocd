...
{
	int retval;
	uint32_t csw, bytes_remaining, max_cnt_noaddrinc;
	bool use_csw_hack = false;
	...
	if (!use_csw_hack) {
		csw = ap->csw_default;
		max_cnt_noaddrinc = STLINK_DATA_SIZE;
	} else {
		/* Hack for FW that do not support CSW. See comment above. */
		csw = STLINK_HLA_CSW;
		max_cnt_noaddrinc = TAR_AUTOINCR_BLOCK - (address & (TAR_AUTOINCR_BLOCK - 1));
	...
	count *= size;
	while (count) {
		bytes_remaining = (count < max_cnt_noaddrinc) ? count : max_cnt_noaddrinc;

		if (!use_csw_hack)
			retval = stlink_usb_read_mem32_noaddrinc(stlink_dap_handle, ap->ap_num,
					csw, address, bytes_remaining, buffer);
		else
