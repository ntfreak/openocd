		...
		uint8_t *data, int data_size, bool check_tcp_status)
{
	...
	int sent_size = send(h->tcp_backend_priv.fd, (char *) cmd, cmd_size, 0);
	...
	int received_size = recv(h->tcp_backend_priv.fd, (char *) data, data_size, 0);
...
static int stlink_tcp_xfer_noerrcheck(void *handle, const uint8_t *buf, int size)
{
	...
	memcpy(&h->tcp_backend_priv.send_buf[8], h->cmdbuf, 16);
	...
	int ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, cmd_size,
			h->tcp_backend_priv.recv_buf, data_size, true);
	if (ret != ERROR_OK)
		return ret;

	if (h->direction != h->tx_ep)
		memcpy((uint8_t *) buf, &h->tcp_backend_priv.recv_buf[STLINK_TCP_SS_SIZE], size);
...
static int stlink_tcp_close(void *handle)
{
	...
	if (h && h->tcp_backend_priv.connected) {
		if (h->tcp_backend_priv.connect_id) {
			...
			h_u32_to_le(&h->tcp_backend_priv.send_buf[4], h->tcp_backend_priv.connect_id);
			ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 8, h->tcp_backend_priv.recv_buf, 4, true);
			if (ret != ERROR_OK)
...
static int stlink_tcp_open(void *handle, struct hl_interface_param_s *param)
{
	...
	if (h->tcp_backend_priv.send_buf == NULL)
		return ERROR_FAIL;

	h->tcp_backend_priv.recv_buf = malloc(STLINK_TCP_RECV_BUFFER_SIZE);
	if (h->tcp_backend_priv.recv_buf == NULL)
		return ERROR_FAIL;
	...
	const int tcp_nodelay_optval = 1;
	res = setsockopt(h->tcp_backend_priv.fd, IPPROTO_TCP, TCP_NODELAY, (const void *) &tcp_nodelay_optval, sizeof(int));
	if (res == -1) {
		LOG_ERROR("cannot set sock option 'TCP_NODELEAY', errno: %s", strerror(errno));
		return ERROR_FAIL;
	}

	const int so_recv_optval = STLINK_TCP_RECV_BUFFER_SIZE;
	res = setsockopt(h->tcp_backend_priv.fd, SOL_SOCKET, SO_RCVBUF, (const void *) &so_recv_optval, sizeof(int));
	if (res == -1) {
		LOG_ERROR("cannot set sock option 'SO_RCVBUF', errno: %s", strerror(errno));
		return ERROR_FAIL;
	}

	const int so_sndbuf_optval = STLINK_TCP_SEND_BUFFER_SIZE;
	res = setsockopt(h->tcp_backend_priv.fd, SOL_SOCKET, SO_SNDBUF, (const void *) &so_sndbuf_optval, sizeof(int));
	if (res == -1) {
	...
	ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 4, h->tcp_backend_priv.recv_buf, 16, false);
	if (ret != ERROR_OK) {
	...
	ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 2, h->tcp_backend_priv.recv_buf, 4, true);
	if (ret != ERROR_OK)
		return ret;

	/* get the number of connected stlinks */
	h->tcp_backend_priv.send_buf[0] = STLINK_TCP_CMD_GET_NB_DEV;
	ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 1, h->tcp_backend_priv.recv_buf, 4, false);
	if (ret != ERROR_OK)
		return ret;
	...
	for (uint32_t stlink_id = 0; stlink_id < connected_stlinks; stlink_id++) {
		...
		ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 8, h->tcp_backend_priv.recv_buf, 45, true);
		if (ret != ERROR_OK)
			return ret;
	...
	ret = stlink_tcp_send_cmd(h, h->tcp_backend_priv.send_buf, 8, h->tcp_backend_priv.recv_buf, 8, true);
	if (ret != ERROR_OK)
		return ret;
