...
static int nulink_usb_version(void *handle)
{
	...
	LOG_INFO("NULINK firmware_version %" PRIu32 ", product_id (0x%08" PRIx32 ")",
			 le_to_h_u32(h->databuf),
			 le_to_h_u32(h->databuf + 4 * 1));

	bool is_nulinkpro = !!(le_to_h_u32(h->databuf + 4 * 2) & 1);
	if (is_nulinkpro) {
		LOG_INFO("NULINK is Nu-Link-Pro, target_voltage_mv(%" PRIu16 "), usb_voltage_mv(%" PRIu16 ")",
				 le_to_h_u16(h->databuf + 4 * 3 + 0),
				 le_to_h_u16(h->databuf + 4 * 3 + 2));

		h->hardware_config |= HARDWARECONFIG_NULINKPRO;
	} else {
...
static int nulink_usb_halt(void *handle)
{
	...
	LOG_DEBUG("NULINK stop_pc 0x%08" PRIx32, le_to_h_u32(h->databuf + 4));
...
static int nulink_usb_step(void *handle)
{
	...
	LOG_DEBUG("NULINK pc 0x%08" PRIx32, le_to_h_u32(h->databuf + 4));
		...
		uint8_t *buffer)
{
	int res = ERROR_OK;
	unsigned int count;
	uint32_t aligned_addr, offset = 0;
		...
		const uint8_t *buffer)
{
	...
	while (len) {
		...
		for (unsigned int i = 0; i < count; i++) {
			...
			h->cmdidx += 4;
			/* u32Data */
			u32buffer = buf_get_u32(buffer, 0, len * 8);
		...
		const uint8_t *buffer)
{
	...
	while (len) {
		...
		for (unsigned int i = 0; i < count; i++) {
			...
			h->cmdidx += 4;
			/* u32Data */
			u32buffer = buf_get_u32(buffer, 0, 32);
			h_u32_to_le(h->cmdbuf + h->cmdidx, u32buffer);
			h->cmdidx += 4;
			/* u32Mask */
...
static int nulink_speed(void *handle, int khz, bool query)
{
	...
	if (!query) {
		...
		if (h->hardware_config & HARDWARECONFIG_NULINKPRO)
			LOG_INFO("NULINK target_voltage_mv[0](%04" PRIx16 "), target_voltage_mv[1](%04" PRIx16
...
static int nulink_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	while (cur_dev) {
		...
		for (int i = 0; param->vid[i] || param->pid[i]; i++) {
	...
	if (target_vid == 0 && target_pid == 0) {
		LOG_ERROR("unable to find NULINK");
		goto error_open;
	}

	hid_device *dev = hid_open(target_vid, target_pid, target_serial);
	if (!dev) {
		LOG_ERROR("unable to open NULINK device 0x%" PRIx16 ":0x%" PRIx16, target_vid, target_pid);
		goto error_open;
	}
	...
	case NULINK2_USB_PID2:
		h->hardware_config = HARDWARECONFIG_NULINK2;
