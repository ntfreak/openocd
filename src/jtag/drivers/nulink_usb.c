...
static int nulink_usb_xfer_rw(void *handle, int cmdsize, uint8_t *buf)
{
	...
	ret = jtag_libusb_interrupt_write(h->fd, h->tx_ep, (char *)h->cmdbuf, cmdsize,
			NULINK_WRITE_TIMEOUT, &transferred);
	if (ret != ERROR_OK)
		return ret;

	ret = jtag_libusb_interrupt_read(h->fd, h->rx_ep, (char *)buf,
			cmdsize, NULINK_READ_TIMEOUT, &transferred);
...
static int nulink_usb_version(void *handle)
{
	...
	LOG_INFO("NULINK firmware_version %d" PRId32 ",product_id (0x%08" PRIx32,
		le_to_h_u32(h->databuf),
		le_to_h_u32(h->databuf + 4 * 1));

	bool isNulinkPro = !!(le_to_h_u32(h->databuf + 4 * 2) & 1);

	if (isNulinkPro) {
		LOG_INFO("NULINK is Nu-Link-Pro, target_voltage_mv(%u), usb_voltage_mv(%u)",
			(le_to_h_u32(h->databuf + 4 * 3)),
			(le_to_h_u32(h->databuf + 4 * 3) >> 16));
...
static int nulink_usb_halt(void *handle)
{
	...
	LOG_DEBUG("NULINK  stop_pc 0x%08" PRIx32,
...
static int nulink_usb_step(void *handle)
{
	...
	LOG_DEBUG("NULINK pc 0x%08x" PRIx32,
		le_to_h_u32(h->databuf + 4));
			  ...
			  uint8_t *buffer)
{
	int res = ERROR_OK;
	unsigned i, count;
	unsigned aligned_addr, offset = 0;
	uint32_t bytes_remaining = 12;
	struct nulink_usb_handle_s *h = handle;

	LOG_DEBUG("nulink_usb_read_mem8: addr 0x%08" PRIx32 ", len %d" PRId32, addr, len);
	...
	if (addr % 4) {
		...
		LOG_DEBUG("nulink_usb_read_mem8: unaligned address addr 0x%08" PRIx32 "/aligned addr 0x%08"
			  PRIx32 "offset %d" PRId32, addr, aligned_addr, offset);
			   ...
			   const uint8_t *buffer)
{
	...
	LOG_DEBUG("nulink_usb_write_mem8: addr 0x%08" PRIx32 ", len %d" PRId32, addr, len);
	...
	if (addr % 4) {
		...
		aligned_addr = aligned_addr * 4;
		offset = addr - aligned_addr;
		LOG_DEBUG("nulink_usb_write_mem8: address not aligned. addr(0x%08x)/aligned_addr(0x%08x)/offset(%d)",
...
static int nulink_speed(void *handle, int khz, bool query)
{
	...
	LOG_DEBUG("NULINK nulink_speed: %lu", max_ice_clock);
	...
	if (!query) {
		...
		if (h->hardwareConfig & 1) {
			LOG_INFO("NULINK target_voltage_mv[0](%04x), target_voltage_mv[1](%04x), target_voltage_mv[2](%04x), if_target_power_supplied(%d)",
				le_to_h_u32(h->databuf + 4 * 1),
				le_to_h_u16(h->databuf + 4 * 1),
				le_to_h_u32(h->databuf + 4 * 2),
				le_to_h_u16(h->databuf + 4 * 2) & 1
				);
		}
	}
...
static int nulink_usb_open(struct hl_interface_param_s *param, void **fd)
{
	...
	do {
		err = jtag_libusb_open(param->vid, param->pid, serial, &h->fd, NULL);
		if (err != ERROR_OK) {
		...
		if (err != ERROR_OK) {
			...
			/* drivers might not binded, proceed */
		} else {
