			  ...
			  uint8_t *buffer)
{
	...
	if (addr % 4) {
		...
		LOG_DEBUG("nulink_usb_read_mem8: address not aligned. addr(0x%08x)/alignedAddr(0x%08x)/offset(%d)",
			   ...
			   const uint8_t *buffer)
{
	...
	if (addr % 4) {
		...
		alignedAddr = alignedAddr * 4;
		offset = addr - alignedAddr;
		LOG_DEBUG("nulink_usb_write_mem8: address not aligned. addr(0x%08x)/alignedAddr(0x%08x)/offset(%d)",
			  addr, alignedAddr, offset);
...
int nulink_config_trace(void *handle, bool enabled,
