/***************************************************************************
 *   Copyright (C) 2020 by Adrian Negreanu                                 *
 *   groleo@gmail.com                                                      *
 *                                                                         *
...
#define DAP_SWO_MIN_BAUDRATE 300
#define DAP_SWO_MAX_BAUDRATE 921600
...
static int cmsis_dap_cmd_DAP_SWJ_Pins(uint8_t pins, uint8_t mask, uint32_t delay, uint8_t *input)
{
	...
	int retval = cmsis_dap_xfer(cmsis_dap_handle, 8);
	if (retval != ERROR_OK) {
...
static int cmsis_dap_cmd_dap_swj_sequence(uint8_t s_len, const uint8_t *sequence)
{
	...
	int retval = cmsis_dap_xfer(cmsis_dap_handle, DIV_ROUND_UP(s_len, 8) + 3);
	if (retval != ERROR_OK || buffer[1] != DAP_OK)
		return ERROR_FAIL;
					...
					uint8_t transport)
{
	...
	int retval = cmsis_dap_xfer(dap, 3);
	if (retval != ERROR_OK || buffer[1] != DAP_OK) {
 ...
 * @param[in]  dev_baudrate Requested baudrate.
					...
					uint32_t *dev_baudrate)
{
	...
	int retval = cmsis_dap_xfer(dap, 6);
					...
					size_t *trace_count)
{
	...
	int retval = cmsis_dap_xfer(dap, 2);
	if (retval != ERROR_OK) {
					...
					uint8_t *data)
{
	...
	int retval = cmsis_dap_xfer(dap, 4);
	if (retval != ERROR_OK) {
				...
				uint16_t *swo_prescaler)
{
	...
	int retval = cmsis_dap_cmd_dap_swo_control(cmsis_dap_handle, DAP_SWO_CONTROL_STOP);
	if (retval != ERROR_OK)
		return retval;
	...
	if (*swo_freq > DAP_SWO_MAX_BAUDRATE) {
		LOG_INFO("Given SWO frequency too high, using %u Hz instead.",
			DAP_SWO_MAX_BAUDRATE);
		*swo_freq = DAP_SWO_MAX_BAUDRATE;
	} else if (*swo_freq < DAP_SWO_MIN_BAUDRATE) {
		LOG_INFO("Given SWO frequency too low, using %u Hz instead.",
			DAP_SWO_MIN_BAUDRATE);
		*swo_freq = DAP_SWO_MIN_BAUDRATE;
	}
...
static int cmsis_dap_poll_trace(uint8_t *buf, size_t *size)
{
	...
	int retval = cmsis_dap_cmd_dap_swo_status(cmsis_dap_handle, &trace_status, &trace_count);
	if (retval != ERROR_OK)
		return retval;
