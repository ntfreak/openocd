...
static char rshim_dev_path[PATH_MAX] = "/dev/rshim0/rshim";
...
{
	rshim_fd = open(rshim_dev_path, O_RDWR | O_SYNC);
	if (rshim_fd == -1) {
...
COMMAND_HANDLER(rshim_dap_device_command)
{
	...
	strncpy(rshim_dev_path, CMD_ARGV[0], sizeof(rshim_dev_path) - 1);
	return ERROR_OK;
}
