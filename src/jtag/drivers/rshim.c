...
struct rshim_sock_msg_hdr {
	uint8_t is_ctrl;/* 0: data, 1: ctrl */
	uint8_t is_ack;	/* 0: request, 1: ack */
...
static int rshim_connect_device(const char *dest)
{
	rshim_fd = open(dest, O_RDWR | O_SYNC);
	if (rshim_fd == -1) {
		LOG_ERROR("Unable to open %s", dest);
		return ERROR_FAIL;
	}
...
static int full_write(int fd, const void *data, int len)
{
	int total = 0;
...
static int rshim_connect_socket(const char *dest)
{
	...
	port_str = strchr(buf, ':');
	if (!port_str) {
		LOG_ERROR("Invalid rshim address %s", dest);
		return ERROR_FAIL;
	}
	if (port_str != buf)
		ip = buf;
	*port_str++ = '\0';
	...
	if (addr.sin_addr.s_addr == INADDR_NONE) {
		LOG_ERROR("Invalid IP %s in rshim address", ip);
		return ERROR_FAIL;
	}
...
{
	const char *dest = rshim_dev_path ?
		(const char *)rshim_dev_path : RSHIM_DEV_PATH_DEFAULT;
