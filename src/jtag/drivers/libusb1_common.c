		...
		adapter_get_alternate_serial_fn adapter_get_alternate_serial)
{
	...
	char *computed_serial = NULL;
	if (adapter_get_alternate_serial &&
			adapter_get_alternate_serial(device, dev_desc, &computed_serial) == ERROR_OK) {
		if (computed_serial && strcmp((const char *) serial, (const char *) computed_serial) == 0) {
