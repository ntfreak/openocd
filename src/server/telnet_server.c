...
static void telnet_auto_complete(struct connection *connection)
{
	...
	for (int i = 0; i < len; i++) {
		...
		if (!ignore_cmd) {
			...
			if (!match) {
				...
				matches_count = 0; /* avoid getting a truncated hints' list */
	...
	if (matches_count == 1) {
		list_for_each_entry(match, &matches, lh)
			break; /* only one iteration is sufficient to get the first and the only match */

		int completion_size = strlen(match->cmd) - usr_cmd_len;
		if (completion_size) {
			/* watch buffer size leaving one spare character for \0 */
			if (t_con->line_size + completion_size >= TELNET_LINE_MAX_SIZE - 1)
				telnet_bell(connection);
			else
				telnet_insert(connection, match->cmd + usr_cmd_len, completion_size);
		}
		...
		while (1) {
			char c = 0;
			list_for_each_entry(match, &matches, lh) {
				if (strlen(match->cmd) == prefix_len)
					goto multiple_matches_completion;

				if (c == 0)
					c = match->cmd[prefix_len];
				else if (match->cmd[prefix_len] != c)
					goto multiple_matches_completion;
				/* else : it's equal */
			}
...
multiple_matches_completion:
		/* if the common part is bigger than user type characters
		 * then auto-complete the common prefix (get it from any matched command)
		 * else list the possible commands */
		if (prefix_len > usr_cmd_len) {
			telnet_insert(connection, match->cmd + usr_cmd_len, prefix_len - usr_cmd_len);
		} else {
			telnet_write(connection, "\n\r", 2);
