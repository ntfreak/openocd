...
static void ipdbg_move_buffer_to_connection(struct connection *conn, struct ipdbg_fifo *fifo)
{
	if (ipdbg_fifo_is_empty(fifo))
		return;
...
static int ipdbg_move_connection_to_fifo(struct connection *conn, struct ipdbg_fifo *fifo)
{
	if (ipdbg_fifo_is_full(fifo))
		return 1;
...
static int ipdbg_distribute_data_from_hub(struct ipdbg_hub *hub, uint32_t up)
{
	...
	if (conn) {
		...
		if (ipdbg_fifo_is_full(&(connection->up_fifo)))
			ipdbg_move_buffer_to_connection(conn, &(connection->up_fifo));
...
static int ipdbg_polling_callback(void *priv)
{
	...
	for (size_t tool = 0 ; tool < hub->max_tools ; ++tool) {
		...
		if (conn && conn->priv) {
			struct ipdbg_connection *connection = conn->priv;
			while (((hub->dn_xoff & (1ul << tool)) == 0) && !ipdbg_fifo_is_empty(&(connection->dn_fifo))) {
				if (ipdbg_jtag_transfer_byte(hub, tool, connection))
	...
	while (empty_up_transfers++ < hub->max_tools) {
		...
		if (retval != ERROR_OK)
			return ret;
		ipdbg_distribute_data_from_hub(hub, up);
	}
	...
	for (size_t tool = 0 ; tool < hub->max_tools ; ++tool) {
		...
		if (conn && conn->priv) {
			...
			if (!ipdbg_fifo_is_empty(&(connection->up_fifo)))
				ipdbg_move_buffer_to_connection(conn, &(connection->up_fifo));
		}
	}
...
static int ipdbg_on_connection_input(struct connection *connection)
{
	...
	int bytes_read = ipdbg_move_connection_to_fifo(connection, &(conn->dn_fifo));
...
static int ipdbg_on_connection_closed(struct connection *connection)
{
	...
	return ipdbg_stop_polling(connection->service->priv, connection);
}
					...
					uint8_t data_register_length, struct ipdbg_virtual_ir_info *virtual_ir, uint8_t tool)
{
	...
	retval = add_service("ipdbg", port_str_buffer, 1, &ipdbg_on_new_connection,
		&ipdbg_on_connection_input, &ipdbg_on_connection_closed, service, NULL);
	if (retval == ERROR_OK) {
