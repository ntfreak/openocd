...
static int ipdbg_move_buffer_to_connection(struct connection *conn, struct ipdbg_fifo *fifo)
{
	...
	ipdbg_zero_rd_idx(fifo);
	int bytes_written = connection_write(conn, fifo->buffer, fifo->count);
	if (bytes_written != (int)(fifo->count)) {
		LOG_ERROR("error during write: %d != %d", (int)bytes_written, (int)fifo->count);
		connection->closed = true;
		return ERROR_SERVER_REMOTE_CLOSED;
	}

	if (bytes_written < (int)(fifo->count)) {
		fifo->rd_idx = bytes_written;
		ipdbg_zero_rd_idx(fifo);
	}
