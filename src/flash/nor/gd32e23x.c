...
struct gd32e23x_flash_bank {
	...
	bool has_dual_banks;
	/* used to access dual flash bank gd32e23x */
...
static int gd32e23x_get_device_id(struct flash_bank *bank, uint32_t *device_id);
static int gd32e23x_write_block(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count);

/* flash bank gd32e23x <base> <size> 0 0 <target#>
 */
FLASH_BANK_COMMAND_HANDLER(gd32e23x_flash_bank_command)
{
	...
	gd32e23x_info->has_dual_banks = false;
	gd32e23x_info->register_base = FLASH_REG_BASE_B0;
	gd32e23x_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static inline int gd32e23x_get_flash_reg(struct flash_bank *bank, uint32_t reg)
{
	struct gd32e23x_flash_bank *gd32e23x_info = bank->driver_priv;
	return reg + gd32e23x_info->register_base;
}

static inline int gd32e23x_get_flash_status(struct flash_bank *bank, uint32_t *status)
{
	struct target *target = bank->target;
	return target_read_u32(target, gd32e23x_get_flash_reg(bank, FMC_STAT), status);
}

static int gd32e23x_wait_status_busy(struct flash_bank *bank, int timeout)
{
	struct target *target = bank->target;
	uint32_t status;
	int retval = ERROR_OK;
	...
	for (;;) {
		retval = gd32e23x_get_flash_status(bank, &status);
		if (retval != ERROR_OK)
			return retval;
	...
	if (status & FMC_STAT_WPERR) {
		LOG_ERROR("gd32e23x device protected");
		retval = ERROR_FAIL;
	}

	if (status & FMC_STAT_PGERR) {
		LOG_ERROR("gd32e23x device programming failed");
		retval = ERROR_FAIL;
	}
	...
	if (status & (FMC_STAT_WPERR | FMC_STAT_PGERR)) {
		 ...
		 * retval
		 */
		target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_STAT),
				FMC_STAT_WPERR | FMC_STAT_PGERR);
	}
	return retval;
}

static int gd32e23x_check_operation_supported(struct flash_bank *bank)
{
	struct gd32e23x_flash_bank *gd32e23x_info = bank->driver_priv;

	/* if we have a dual flash bank device then
	 * we need to perform option byte stuff on bank0 only */
	if (gd32e23x_info->register_base != FLASH_REG_BASE_B0) {
...
static int gd32e23x_read_options(struct flash_bank *bank)
{
	uint32_t optiondata;
	...
	gd32e23x_info->option_bytes.user_options = (optiondata >> gd32e23x_info->option_offset >> 2) & 0xffff;
	gd32e23x_info->option_bytes.user_data = (optiondata >> gd32e23x_info->user_data_offset) & 0xffff;
	gd32e23x_info->option_bytes.RDP = (optiondata & (1 << FMC_OBSTAT_SPC)) ? 0xFFFF : 0x5AA5;
	...
	gd32e23x_info->option_bytes.protection[0] = (uint16_t)optiondata;
	gd32e23x_info->option_bytes.protection[1] = (uint16_t)(optiondata >> 8);
	gd32e23x_info->option_bytes.protection[2] = (uint16_t)(optiondata >> 16);
	gd32e23x_info->option_bytes.protection[3] = (uint16_t)(optiondata >> 24);

	return ERROR_OK;
}

static int gd32e23x_erase_options(struct flash_bank *bank)
{
	struct gd32e23x_flash_bank *gd32e23x_info = NULL;
	struct target *target = bank->target;
	...
	retval = gd32e23x_wait_status_busy(bank, FLASH_ERASE_TIMEOUT);
	if (retval != ERROR_OK)
		return retval;

	/* clear readout protection and complementary option bytes
	 * this will also force a device unlock if set */
	gd32e23x_info->option_bytes.RDP = gd32e23x_info->default_rdp;

	return ERROR_OK;
}

static int gd32e23x_write_options(struct flash_bank *bank)
{
	struct gd32e23x_flash_bank *gd32e23x_info = NULL;
	struct target *target = bank->target;
	...
	target_buffer_set_u16(target, opt_bytes, gd32e23x_info->option_bytes.RDP);
	target_buffer_set_u16(target, opt_bytes + 2, gd32e23x_info->option_bytes.user_options);
	target_buffer_set_u16(target, opt_bytes + 4, gd32e23x_info->option_bytes.user_data & 0xff);
	target_buffer_set_u16(target, opt_bytes + 6, (gd32e23x_info->option_bytes.user_data >> 8) & 0xff);
	target_buffer_set_u16(target, opt_bytes + 8, gd32e23x_info->option_bytes.protection[0]);
	target_buffer_set_u16(target, opt_bytes + 10, gd32e23x_info->option_bytes.protection[1]);
	target_buffer_set_u16(target, opt_bytes + 12, gd32e23x_info->option_bytes.protection[2]);
	target_buffer_set_u16(target, opt_bytes + 14, gd32e23x_info->option_bytes.protection[3]);

	uint32_t offset = FMC_OB_RDP - bank->base;
	retval = gd32e23x_write_block(bank, opt_bytes, offset, sizeof(opt_bytes) / 2);
	if (retval != ERROR_OK) {
...
static int gd32e23x_protect_check(struct flash_bank *bank)
{
	...
	int retval = gd32e23x_check_operation_supported(bank);
	if (ERROR_OK != retval)
		return retval;
	...
	num_bits = (bank->num_sectors / gd32e23x_info->ppage_size);
		...
		for (i = 0; i < num_bits; i++) {
			...
			for (s = 0; s < gd32e23x_info->ppage_size; s++)
				bank->sectors[(i * gd32e23x_info->ppage_size) + s].is_protected = set;
		}

	return ERROR_OK;
}

static int gd32e23x_erase(struct flash_bank *bank, int first, int last)
{
	struct target *target = bank->target;
	int i;
	uint32_t optiondata;
	...
	if ((0xFFFFFFFF != optiondata) || ((obstat & 0x2) != 0)) {
		gd32e23x_erase_options(bank);
		optiondata = 0xFFFFFFFF;
		gd32e23x_info->option_bytes.RDP = 0x5AA5;
		gd32e23x_info->option_bytes.protection[0] = (uint16_t)optiondata;
		gd32e23x_info->option_bytes.protection[1] = (uint16_t)(optiondata >> 8);
		gd32e23x_info->option_bytes.protection[2] = (uint16_t)(optiondata >> 16);
		gd32e23x_info->option_bytes.protection[3] = (uint16_t)(optiondata >> 24);

		gd32e23x_write_options(bank);
	...
	/* unlock flash registers */
	int retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_KEY), UNLOCK_KEY0);
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_KEY), UNLOCK_KEY1);
	if (retval != ERROR_OK)
		return retval;

	for (i = first; i <= last; i++) {
		retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_PER);
		if (retval != ERROR_OK)
			return retval;
		retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_ADDR),
				bank->base + bank->sectors[i].offset);
		if (retval != ERROR_OK)
			return retval;
		retval = target_write_u32(target,
				gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_PER | FMC_CTL_START);
		if (retval != ERROR_OK)
			return retval;
	...
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_LK);
	if (retval != ERROR_OK)
		return retval;
...
static int gd32e23x_protect(struct flash_bank *bank, int set, int first, int last)
{
	struct gd32e23x_flash_bank *gd32e23x_info = NULL;
	struct target *target = bank->target;
	...
	if ((first % gd32e23x_info->ppage_size) != 0) {
		LOG_WARNING("aligned start protect sector to a %d sector boundary",
				gd32e23x_info->ppage_size);
		first = first - (first % gd32e23x_info->ppage_size);
	}
	if (((last + 1) % gd32e23x_info->ppage_size) != 0) {
		LOG_WARNING("aligned end protect sector to a %d sector boundary",
				gd32e23x_info->ppage_size);
		last++;
		last = last - (last % gd32e23x_info->ppage_size);
		last--;
	}
	...
	prot_reg[3] = (uint16_t)(protection >> 24);
		...
		for (i = first; i <= last; i++) {
			reg = (i / gd32e23x_info->ppage_size) / 8;
			bit = (i / gd32e23x_info->ppage_size) - (reg * 8);
	...
	status = gd32e23x_erase_options(bank);
	if (status != ERROR_OK)
		return status;

	gd32e23x_info->option_bytes.protection[0] = prot_reg[0];
	gd32e23x_info->option_bytes.protection[1] = prot_reg[1];
	gd32e23x_info->option_bytes.protection[2] = prot_reg[2];
	gd32e23x_info->option_bytes.protection[3] = prot_reg[3];

	return gd32e23x_write_options(bank);
}
		...
		uint32_t offset, uint32_t count)
{
	...
	static const uint8_t gd32e23x_flash_write_code[] = {
		/* #define GD32_FLASH_CTL_OFFSET 0x0C */
		/* wait_fifo: */
	...
	buf_set_u32(reg_params[0].value, 0, 32, gd32e23x_info->register_base);
	...
	if (retval == ERROR_FLASH_OPERATION_FAILED) {
		...
		if (buf_get_u32(reg_params[0].value, 0, 32) & FMC_STAT_PGERR) {
			...
			/* Clear but report errors */
			target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_STAT), FMC_STAT_PGERR);
		}

		if (buf_get_u32(reg_params[0].value, 0, 32) & FMC_STAT_WPERR) {
			LOG_ERROR("flash memory write protected");
			/* Clear but report errors */
			target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_STAT), FMC_STAT_WPERR);
		}
	}
		...
		uint32_t offset, uint32_t count)
{
	...
	/* unlock flash registers */
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_KEY), UNLOCK_KEY0);
	if (retval != ERROR_OK)
		goto cleanup;
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_KEY), UNLOCK_KEY1);
	if (retval != ERROR_OK)
	...
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_PG);
	if (retval != ERROR_OK)
		goto cleanup;

	/* try using a block write */
	retval = gd32e23x_write_block(bank, buffer, offset, words_remaining);
	...
	if (retval == ERROR_TARGET_RESOURCE_NOT_AVAILABLE) {
		...
		while (words_remaining > 0) {
			...
			retval = gd32e23x_wait_status_busy(bank, 5);
			if (retval != ERROR_OK)
				goto reset_pg_and_lock;
...
reset_pg_and_lock:
	retval2 = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_LK);
	if (retval == ERROR_OK)
		retval = retval2;
...
static int gd32e23x_get_device_id(struct flash_bank *bank, uint32_t *device_id)
{
	...
	if (((cpuid >> 4) & 0xFFF) == 0xD20) {
		...
		LOG_ERROR("Cannot identify target as a gd32e23x");
		return ERROR_FAIL;
	}

	/* read gd32 device id register */
...
static int gd32e23x_get_flash_size(struct flash_bank *bank, uint16_t *flash_size_in_kb)
{
	struct target *target = bank->target;
...
static int gd32e23x_probe(struct flash_bank *bank)
{
	...
	gd32e23x_info->probed = 0;
	gd32e23x_info->register_base = FLASH_REG_BASE_B0;
	gd32e23x_info->user_data_offset = 10;
	gd32e23x_info->option_offset = 0;

	/* default factory protection level */
	gd32e23x_info->default_rdp = 0x5AA5;

	/* read gd32 device id register */
	int retval = gd32e23x_get_device_id(bank, &device_id);
	if (retval != ERROR_OK)
		return retval;
	...
	default:
		LOG_WARNING("Cannot identify target as a GD32 family.");
		return ERROR_FAIL;
	}

	/* get flash size from target. */
	retval = gd32e23x_get_flash_size(bank, &flash_size_in_kb);

	/* failed reading flash size or flash size invalid (early silicon),
	 * default to max target family */
	if (retval != ERROR_OK || flash_size_in_kb == 0xffff || flash_size_in_kb == 0) {
		LOG_WARNING("GD32 flash size failed, probe inaccurate - assuming %dk flash",
	...
	if (gd32e23x_info->user_bank_size) {
		LOG_INFO("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = gd32e23x_info->user_bank_size / 1024;
	}
...
static int get_gd32e23x_info(struct flash_bank *bank, char *buf, int buf_size)
{
	uint32_t dbgmcu_idcode;

		/* read gd32 device id register */
	int retval = gd32e23x_get_device_id(bank, &dbgmcu_idcode);
	if (retval != ERROR_OK)
		return retval;
	...
	default:
		snprintf(buf, buf_size, "Cannot identify target as a GD32E23x\n");
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(gd32e23x_handle_lock_command)
{
	...
	retval = gd32e23x_check_operation_supported(bank);
	if (ERROR_OK != retval)
		return retval;

	if (gd32e23x_erase_options(bank) != ERROR_OK) {
		command_print(CMD, "gd32e23x failed to erase options");
		return ERROR_OK;
	}

	/* set readout protection */
	gd32e23x_info->option_bytes.RDP = 0;

	if (gd32e23x_write_options(bank) != ERROR_OK) {
...
COMMAND_HANDLER(gd32e23x_handle_options_read_command)
{
	uint32_t optionbyte;
	struct target *target = NULL;
	struct gd32e23x_flash_bank *gd32e23x_info = NULL;
	...
	optionbyte >>= gd32e23x_info->option_offset;
	...
	if (gd32e23x_info->has_dual_banks) {
	...
	command_print(CMD, "User Option0: 0x%02" PRIx8,
			(uint8_t)((user_data >> gd32e23x_info->user_data_offset) & 0xff));
	command_print(CMD, "User Option1: 0x%02" PRIx8,
			(uint8_t)((user_data >> (gd32e23x_info->user_data_offset + 8)) & 0xff));

	return ERROR_OK;
}

COMMAND_HANDLER(gd32e23x_handle_options_write_command)
{
	struct target *target = NULL;
	struct gd32e23x_flash_bank *gd32e23x_info = NULL;
	uint16_t optionbyte;
	...
	retval = gd32e23x_read_options(bank);
	if (ERROR_OK != retval)
		return retval;

	/* start with current options */
	optionbyte = gd32e23x_info->option_bytes.user_options;
	...
	while (CMD_ARGC) {
		if (strcmp("SWWDG", CMD_ARGV[0]) == 0)
			...
			optionbyte &= ~(1 << 2);
		else if (gd32e23x_info->has_dual_banks) {
	...
	gd32e23x_info->option_bytes.user_options = optionbyte;
...
static int gd32e23x_mass_erase(struct flash_bank *bank)
{
	...
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL), FMC_CTL_MER);
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(target, gd32e23x_get_flash_reg(bank, FMC_CTL),
...
static const struct command_registration gd32e23x_exec_command_handlers[] = {
	...
	{
		.name = "options_read",
		.handler = gd32e23x_handle_options_read_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
	...
	{
		.name = "options_write",
		.handler = gd32e23x_handle_options_write_command,
		.mode = COMMAND_EXEC,
...
struct flash_driver gd32e23x_flash = {
	.name = "gd32e23x",
