...
struct stm32h7x_part_info {
	...
	const unsigned int block_size; /* flash write word size in bytes */
...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	/* setup bank size */
	uint32_t bank1_base = FLASH_BANK0_ADDRESS;
	uint32_t bank2_base = bank1_base + stm32x_info->part_info->max_bank_size_kb * 1024;
	...
	case 0x480:
		 ...
		 */
		bank2_base = bank1_base + stm32x_info->part_info->max_bank_size_kb * 1024;
	...
	if (has_dual_bank) {
		...
		if (bank->base == bank2_base) {
			LOG_ERROR("this device have a single bank only");
			return ERROR_FAIL;
