...
static int rp2040_flash_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	const unsigned int erase_sectors_per_call = 4 * BLOCK_SIZE / priv->dev->sectorsize;
...
static int ssel_active(struct target *target, bool active)
{
