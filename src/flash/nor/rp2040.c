...
static int rp2040_flash_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	...
	LOG_DEBUG("Allocated flash bounce buffer @0x%" PRIx32, (unsigned)bounce->address);
...
static int rp2040_flash_probe(struct flash_bank *bank)
{
	...
	if (!priv->dev) {
		LOG_ERROR("Unknown flash device (ID 0x%" PRIx32 ")", device_id);
		return ERROR_FAIL;
	}

	LOG_INFO("Found flash device \'%s\' (ID 0x%" PRIx32 ")",
	...
	LOG_INFO("RP2040 B0 Flash Probe: %d bytes @0x%" PRIx32 ", in %d sectors\n",
