...
COMMAND_HANDLER(gd32vf103_handle_lock_command)
{
	...
	if (gd32vf103_erase_options(bank) != ERROR_OK) {
		command_print(CMD_CTX, "gd32vf103 failed to erase options");
		return ERROR_OK;
	}
	...
	if (gd32vf103_write_options(bank) != ERROR_OK) {
		command_print(CMD_CTX, "gd32vf103 failed to lock device");
		return ERROR_OK;
	}

	command_print(CMD_CTX, "gd32vf103 locked");
...
COMMAND_HANDLER(gd32vf103_handle_unlock_command)
{
	...
	if (gd32vf103_erase_options(bank) != ERROR_OK) {
		command_print(CMD_CTX, "gd32vf103 failed to unlock device");
		return ERROR_OK;
	}
	...
	command_print(CMD_CTX, "gd32vf103 unlocked.\n"
...
COMMAND_HANDLER(gd32vf103_handle_options_write_command)
{
	...
	if (gd32vf103_write_options(bank) != ERROR_OK) {
		command_print(CMD_CTX, "gd32vf103 failed to write options");
		return ERROR_OK;
	}

	command_print(CMD_CTX, "gd32vf103 write options complete.\n"
				"INFO: a reset or power cycle is required "
				"for the new settings to take effect.");
...
COMMAND_HANDLER(gd32vf103_handle_mass_erase_command)
{
	...
	if (retval == ERROR_OK) {
		...
		command_print(CMD_CTX, "gd32vf103 mass erase complete");
	} else
		command_print(CMD_CTX, "gd32vf103 mass erase failed");
