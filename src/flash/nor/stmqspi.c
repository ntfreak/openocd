...
static int qspi_write_enable(struct flash_bank *bank)
{
	...
	if ((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH)))
		...
		if ((status & (SPIFLASH_WE_BIT | SPIFLASH_BSY_BIT)) != SPIFLASH_WE_BIT) {
			LOG_ERROR("Cannot write enable flash1. Status=0x%02" PRIx32,
				status & 0xFFU);
			return ERROR_FLASH_OPERATION_FAILED;
		}
	...
	if ((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH))) != 0)
		if ((status & (SPIFLASH_WE_BIT | SPIFLASH_BSY_BIT)) != SPIFLASH_WE_BIT) {
			LOG_ERROR("Cannot write enable flash2. Status=0x%02" PRIx32,
				status & 0xFFU);
			return ERROR_FLASH_OPERATION_FAILED;
		}
...
COMMAND_HANDLER(stmqspi_handle_mass_erase_command)
{
	...
	if (((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH)))
		...
		LOG_ERROR("Mass erase command not accepted by flash1. Status=0x%02" PRIx32,
			status & 0xFFU);
	...
	if (((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH))) != 0) &&
		...
		((status & SPIFLASH_WE_BIT) != 0)) {
		LOG_ERROR("Mass erase command not accepted by flash2. Status=0x%02" PRIx32,
			status & 0xFFU);
		retval = ERROR_FLASH_OPERATION_FAILED;
		goto err;
	}
...
static int qspi_erase_sector(struct flash_bank *bank, unsigned int sector)
{
	...
	if (((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH)))
		!= BIT(SPI_FSEL_FLASH)) && ((status & SPIFLASH_BSY_BIT) == 0) &&
		((status & SPIFLASH_WE_BIT) != 0)) {
		LOG_ERROR("Sector erase command not accepted by flash1. Status=0x%02" PRIx32,
			status & 0xFFU);
		retval = ERROR_FLASH_OPERATION_FAILED;
		goto err;
	}
	...
	if (((stmqspi_info->saved_cr & (BIT(SPI_DUAL_FLASH) | BIT(SPI_FSEL_FLASH))) != 0) &&
		((status & SPIFLASH_BSY_BIT) == 0) &&
		((status & SPIFLASH_WE_BIT) != 0)) {
		LOG_ERROR("Sector erase command not accepted by flash2. Status=0x%02" PRIx32,
			status & 0xFFU);
		retval = ERROR_FLASH_OPERATION_FAILED;
		goto err;
	}
...
static int stmqspi_blank_check(struct flash_bank *bank)
{
	...
	while (sector < bank->num_sectors) {
		...
		for (index = 0; index < count; index++) {
			...
			LOG_DEBUG("Flash sector %u checked: 0x%04" PRIx32, sector + index, result & 0xFFFF);
		}
		keep_alive();
		sector += count;
	}
