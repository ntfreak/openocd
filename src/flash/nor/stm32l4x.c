...
static int stm32l4_update_secbb(struct flash_bank *bank, uint32_t value)
{
	int retval;
		...
		unsigned int last)
{
	...
	if (stm32l4_info->part_info->flags & F_HAS_TZ && stm32l4_info->tzen && (stm32l4_info->rdp == RDP_LEVEL_0))
		stm32l4_update_secbb(bank, 0xffffffff);
