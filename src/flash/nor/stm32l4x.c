...
static int stm32l4_protect_check(struct flash_bank *bank)
{
	...
	stm32l4_read_flash_reg(bank, STM32_FLASH_WRP1AR, REG_INDEX, &wrp1ar);
	stm32l4_read_flash_reg(bank, STM32_FLASH_WRP1BR, REG_INDEX, &wrp1br);
	if (stm32l4_info->part_info->has_dual_bank) {
		stm32l4_read_flash_reg(bank, STM32_FLASH_WRP2AR, REG_INDEX, &wrp2ar);
		stm32l4_read_flash_reg(bank, STM32_FLASH_WRP2BR, REG_INDEX, &wrp2br);
	} else {
...
static int stm32l4_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	if (last >= stm32l4_info->bank1_sectors) {
		...
		ret = stm32l4_write_option(bank, STM32_FLASH_WRP2AR, REG_INDEX, reg_value, 0xffffffff);
	}
	/* Bank 1 */
	...
	if (first < stm32l4_info->bank1_sectors) {
		...
		ret = stm32l4_write_option(bank, STM32_FLASH_WRP1AR, REG_INDEX, reg_value, 0xffffffff);
	}
