...
static inline bool stm32l4_has_dual_bank(struct flash_bank *bank)
{
	...
	return (stm32l4_info->part_info->optr_mask & (OPT_DB1M | OPT_DBANK)) != 0;
}
...
static int stm32l4_probe(struct flash_bank *bank)
{
	...
	/* initialize bank1_sectors and hole_sectors for single bank devices */
	...
	if (stm32l4_has_dual_bank(bank)) {
		...
		options = options & part_info->optr_mask; /* reset useless bits */
		if (retval != ERROR_OK)
			return retval;

		/* check for OPT_DBANK, if it is set, the page size is multiplied by 2 */
		if ((options & OPT_DBANK) == 0)
			stm32l4_info->page_size *= 2;

		if ((options & OPT_DB1M) && (flash_size_in_kb < part_info->max_flash_size_kb)) {
			/* FIXME, need to be checked */
			stm32l4_info->bank1_sectors = ((flash_size_in_kb * 1024) / stm32l4_info->page_size) / 2;
			stm32l4_info->hole_sectors = (((flash_size_in_kb * 1024) / stm32l4_info->page_size) / 2)
					- stm32l4_info->bank1_sectors;
		}
	}
