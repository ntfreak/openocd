		...
		unsigned int last)
{
	...
	if (stm32l4_info->tzen && (stm32l4_info->rdp == RDP_LEVEL_0)) {
		...
		if (retval != ERROR_OK) {
			LOG_ERROR("failed to set page block-based security attribution");
...
err_lock:
	...
	if (stm32l4_info->tzen && (stm32l4_info->rdp == RDP_LEVEL_0)) {
		...
		int retval3 = stm32l4_set_secbb(bank, 0);
		if (retval3 != ERROR_OK) {
			LOG_ERROR("failed to clear page block-based security attribution");
			return retval3;
		}
	}
...
COMMAND_HANDLER(stm32l4_handle_flashloader_command)
{
	...
	LOG_INFO("FlashLoader usage is %s", stm32l4_info->use_flashloader ? "enabled" : "disabled");
