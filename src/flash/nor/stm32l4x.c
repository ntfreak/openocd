		...
		struct stm32l4_wrp *wrpxy, unsigned int *n_wrp)
{
	...
	*n_wrp = 0;
	...
	if (dev_bank_id != STM32_BANK2) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP1AR_INDEX, .offset = 0};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP1BR_INDEX, .offset = 0};
	...
	if (wrp2y_sectors_offset > -1) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP2AR_INDEX, .offset = wrp2y_sectors_offset};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP2BR_INDEX, .offset = wrp2y_sectors_offset};
	}

	/* read available WRPxx */
	for (unsigned int i = 0; i < *n_wrp; i++) {
		ret = stm32l4_read_flash_reg_by_index(bank, wrpxy[i].reg_idx, &wrpxy[i].value);
		if (ret != ERROR_OK)
			return ret;
...
COMMAND_HANDLER(stm32l4_handle_unlock_command)
{
	...
	struct stm32l4_flash_bank *stm32l4_info = bank->driver_priv;
	if (stm32l4_write_option(bank, stm32l4_info->flash_regs[STM32_FLASH_OPTR_INDEX], RDP_LEVEL_0, 0x000000FF) != ERROR_OK) {
