...
static const uint32_t stm32l4_flash_regs[STM32_FLASH_REG_NUM] = {
	[STM32_FLASH_ACR]      = 0x000,
	[STM32_FLASH_KEYR]     = 0x008,
	[STM32_FLASH_OPTKEYR]  = 0x00C,
	[STM32_FLASH_SR]       = 0x010,
	[STM32_FLASH_CR]       = 0x014,
	[STM32_FLASH_OPTR]     = 0x020,
	[STM32_FLASH_WRP1AR]   = 0x02C,
	[STM32_FLASH_WRP1BR]   = 0x030,
	[STM32_FLASH_WRP2AR]   = 0x04C,
	[STM32_FLASH_WRP2BR]   = 0x050,
};

static const uint32_t stm32l5_ns_flash_regs[STM32_FLASH_REG_NUM] = {
	[STM32_FLASH_ACR]      = 0x000,
	[STM32_FLASH_KEYR]     = 0x008,
	[STM32_FLASH_OPTKEYR]  = 0x010,
	[STM32_FLASH_SR]       = 0x020,
	[STM32_FLASH_CR]       = 0x028,
	[STM32_FLASH_OPTR]     = 0x040,
	[STM32_FLASH_WRP1AR]   = 0x058,
	[STM32_FLASH_WRP1BR]   = 0x05C,
	[STM32_FLASH_WRP2AR]   = 0x068,
	[STM32_FLASH_WRP2BR]   = 0x06C,
};
...
{
	struct stm32l4_flash_bank *stm32l4_info = bank->driver_priv;
	return stm32l4_info->part_info->flash_regs_base +
			(reg_param_type == REG_INDEX ? stm32l4_info->flash_regs[reg] : reg);
}

static inline int stm32l4_read_flash_reg(struct flash_bank *bank, uint32_t reg,
...
static inline int stm32l4_write_flash_reg(struct flash_bank *bank, uint32_t reg,
	enum reg_param_type reg_param_type, uint32_t value)
{
	return target_write_u32(bank->target, stm32l4_get_flash_reg(bank, reg, reg_param_type), value);
}
...
static int stm32l4_wait_status_busy(struct flash_bank *bank, int timeout)
{
	...
	for (;;) {
		retval = stm32l4_read_flash_reg(bank, STM32_FLASH_SR, REG_INDEX, &status);
		if (retval != ERROR_OK)
			return retval;
	...
	if (status & FLASH_ERROR) {
		 ...
		 * retval
		 */
		stm32l4_write_flash_reg(bank, STM32_FLASH_SR, REG_INDEX, status & FLASH_ERROR);
	}
...
static int stm32l4_unlock_reg(struct flash_bank *bank)
{
	 ...
	 */
	int retval = stm32l4_read_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, &ctrl);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_KEYR, REG_INDEX, KEY1);
	if (retval != ERROR_OK)
		return retval;

	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_KEYR, REG_INDEX, KEY2);
	if (retval != ERROR_OK)
		return retval;

	retval = stm32l4_read_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, &ctrl);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l4_unlock_option_reg(struct flash_bank *bank)
{
	...
	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_OPTKEYR, REG_INDEX, OPTKEY1);
	if (retval != ERROR_OK)
		return retval;

	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_OPTKEYR, REG_INDEX, OPTKEY2);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l4_write_option(struct flash_bank *bank, uint32_t reg,
	enum reg_param_type reg_param_type,	uint32_t value, uint32_t mask,
	enum stm32l4_obl_launch obl_launch)
{
	uint32_t optiondata;
	int retval, retval2;

	retval = stm32l4_read_flash_reg(bank, reg, reg_param_type, &optiondata);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = stm32l4_write_flash_reg(bank, reg, reg_param_type, optiondata);
	if (retval != ERROR_OK)
		goto err_lock;

	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, FLASH_OPTSTRT);
	if (retval != ERROR_OK)
		goto err_lock;
	...
	if (obl_launch == OBL_LAUNCH_PERFORM) {
		stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, FLASH_OBL_LAUNCH);
		return ERROR_OK;
	}

err_lock:
	retval2 = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, FLASH_LOCK | FLASH_OPTLOCK);
...
static int stm32l4_get_wrpxy(struct flash_bank *bank, enum stm32_bank_id dev_bank_id,
		struct stm32l4_wrp *wrpxy, int *n_wrp)
{
	...
	*n_wrp = 0;
	...
	if (dev_bank_id != STM32_BANK2) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP1AR, .offset = 0};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP1BR, .offset = 0};
	...
	if (wrp2y_sectors_offset > -1) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP2AR, .offset = wrp2y_sectors_offset};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_idx = STM32_FLASH_WRP2BR, .offset = wrp2y_sectors_offset};
	}

	/* read available WRPxx */
	for (int i = 0; i < *n_wrp; i++) {
		ret = stm32l4_read_flash_reg(bank, wrpxy[i].reg_idx, REG_INDEX, &wrpxy[i].value);
		if (ret != ERROR_OK)
			return ret;

		wrpxy[i].first = (wrpxy[i].value & stm32l4_info->wrpxxr_mask) + wrpxy[i].offset;
		wrpxy[i].last = ((wrpxy[i].value >> 16) & stm32l4_info->wrpxxr_mask) + wrpxy[i].offset;
...
static int stm32l4_erase(struct flash_bank *bank, int first, int last)
{
	...
	for (i = first; i <= last; i++) {
		...
		retval = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, erase_flags);
		if (retval != ERROR_OK)
			break;
...
err_lock:
	retval2 = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, FLASH_LOCK);
...
static int stm32l4_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	ret = stm32l4_get_wrpxy(bank, use_bank2 ? STM32_BANK2 : STM32_BANK1, wrpxy, &n_wrp);
	if (ret != ERROR_OK)
		return ret;
	...
	for (i = 0; i < n_wrp; i++) {
		...
		ret = stm32l4_write_option(bank, wrpxy[i].reg_idx, REG_INDEX, wrp_value,
				0xffffffff, OBL_LAUNCH_POSTPONE);
...
#include "../../../contrib/loaders/flash/stm32/stm32l4x.inc"
	...
	buf_set_u32(reg_params[4].value, 0, 32, stm32l4_get_flash_reg(bank, STM32_FLASH_SR, REG_INDEX));
	buf_set_u32(reg_params[5].value, 0, 32, stm32l4_get_flash_reg(bank, STM32_FLASH_CR, REG_INDEX));
	...
	if (retval == ERROR_FLASH_OPERATION_FAILED) {
		...
		if (error != 0) {
			...
			/* Clear but report errors */
			stm32l4_write_flash_reg(bank, STM32_FLASH_SR, REG_INDEX, error);
			retval = ERROR_FAIL;
		}
	}
...
static int get_stm32l4_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	if (part_info) {
		...
		snprintf(buf, buf_size, "%s - Rev %s : 0x%04x",
...
static int stm32l4_mass_erase(struct flash_bank *bank)
{
	...
	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, action);
	if (retval != ERROR_OK)
		goto err_lock;

	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, action | FLASH_STRT);
	if (retval != ERROR_OK)
		goto err_lock;
...
COMMAND_HANDLER(stm32l4_handle_option_read_command)
{
	...
	reg_addr = stm32l4_get_flash_reg(bank, reg_offset, REG_OFFSET);

	retval = stm32l4_read_flash_reg(bank, reg_offset, REG_OFFSET, &value);
	if (ERROR_OK != retval)
		return retval;
...
COMMAND_HANDLER(stm32l4_handle_option_write_command)
{
	...
	retval = stm32l4_write_option(bank, reg_offset, REG_OFFSET, value, mask, OBL_LAUNCH_POSTPONE);
	return retval;
}

COMMAND_HANDLER(stm32l4_handle_tzen_command)
{
	...
	uint32_t optr;
	retval = stm32l4_read_flash_reg(bank, STM32_FLASH_OPTR, REG_INDEX, &optr);
	if (retval != ERROR_OK)
		return retval;
	...
	if (new_tzen) {
		...
		retval = stm32l4_write_option(bank, STM32_FLASH_OPTR, REG_INDEX,
...
COMMAND_HANDLER(stm32l4_handle_option_load_command)
{
	 ...
	 */
	retval = stm32l4_write_flash_reg(bank, STM32_FLASH_CR, REG_INDEX, FLASH_OBL_LAUNCH);
...
COMMAND_HANDLER(stm32l4_handle_lock_command)
{
	...
	if (stm32l4_write_option(bank, STM32_FLASH_OPTR, REG_INDEX, RDP_LEVEL_1,
			FLASH_RDP_MASK, OBL_LAUNCH_POSTPONE) != ERROR_OK) {
...
COMMAND_HANDLER(stm32l4_handle_wrp_desc_command)
{
	...
	ret = stm32l4_get_wrpxy(bank, dev_bank_id, wrpxy, &n_wrp);
	if (ret != ERROR_OK)
		return ret;
...
static const struct command_registration stm32l4_exec_command_handlers[] = {
	...
	{
		.name = "tzen",
		.handler = stm32l4_handle_tzen_command,
		.mode = COMMAND_EXEC,
		.usage = "<bank_id> [enable|disable]",
		.help = "Configure TZEN option bit",
	},
	{
		.name = "wrp_desc",
