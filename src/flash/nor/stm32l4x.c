		...
		struct stm32l4_wrp *wrpxy, int *n_wrp)
{
	...
	*n_wrp = 0;
	...
	if (dev_bank_id != STM32_BANK2) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_offset = STM32_FLASH_WRP1AR, .offset = 0};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_offset = STM32_FLASH_WRP1BR, .offset = 0};
	...
	if (wrp2y_sectors_offset > -1) {
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_offset = STM32_FLASH_WRP2AR, .offset = wrp2y_sectors_offset};
		wrpxy[(*n_wrp)++] = (struct stm32l4_wrp){.reg_offset = STM32_FLASH_WRP2BR, .offset = wrp2y_sectors_offset};
	}

	/* read available WRPxx */
	for (int i = 0; i < *n_wrp; i++) {
		ret = stm32l4_read_flash_reg(bank, wrpxy[i].reg_offset, &wrpxy[i].value);
		if (ret != ERROR_OK)
			return ret;
...
static int stm32l4_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	for (i = 0; i < n_wrp; i++) {
		...
		ret = stm32l4_write_option(bank, wrpxy[i].reg_offset, wrp_value, 0x00ff00ff);
