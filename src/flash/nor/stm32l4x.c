...
static inline uint32_t stm32l4_get_flash_reg(struct flash_bank *bank, uint32_t reg_idx)
{
	struct stm32l4_flash_bank *stm32l4_info = bank->driver_priv;
	return stm32l4_info->part_info->flash_regs_base + stm32l4_info->flash_regs[reg_idx];
}

static inline int stm32l4_read_flash_reg(struct flash_bank *bank, uint32_t reg_idx, uint32_t *value)
{
	return target_read_u32(bank->target, stm32l4_get_flash_reg(bank, reg_idx), value);
}

static inline int stm32l4_write_flash_reg(struct flash_bank *bank, uint32_t reg_idx, uint32_t value)
{
	return target_write_u32(bank->target, stm32l4_get_flash_reg(bank, reg_idx), value);
}
