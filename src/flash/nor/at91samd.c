...
#define SAMD_MAIN_FLASH		((uint32_t)0x00000000)	/* physical NVM Flash memory */
...
static int samd_determine_flash_region(const struct samd_info *info, uint32_t address)
{
	int flashregion = kFlashStorageAreaNVM;

	if (address == SAMD_USER_ROW)
		flashregion = kFlashStorageAreaUserRow;
	else if (info->rwwee_start_address != 0 && address >= info->rwwee_start_address)
		...
		uint32_t *sizep, int *nvmnump, int *rwweenump)
{
	...
	if (res == ERROR_OK) {
		...
		/* The NVMP field (bits 15:0) indicates the total number of NVM pages */
		if (nvmnump)
...
static int samd_probe(struct flash_bank *bank)
{
	 ...
	 * actually has an rwwee section. But we want samd_determine_flash_region
	 * to indicate the address is in the rwwee region even if the chip doesn't
	 * have rwwee. */
	...
	switch (regionnumber) {
		case kFlashStorageAreaNVM: {
			...
			bankname = "NVM";
			protectionblocks = SAMD_NUM_PROT_BLOCKS; // 16 protection blocks
		}
		break;
	...
	if (bank->size != operativepages * chip->page_size) {
		LOG_WARNING("SAMD: bank size doesn't match %s parameters. "
				...
				bankname, sizeinkbstr, operativepages, chip->page_size);
	}
	...
	if (protectionblocks != 0) {
		...
		bank->prot_blocks = alloc_block_array(0, chip->prot_block_size, protectionblocks);
		if (!bank->prot_blocks)
			return ERROR_FAIL;
	}
	else {
		...
		uint32_t offset, uint32_t count)
{
	...
	while (count) {
		...
		if (manual_wp || pg_offset + 4 * nw < chip->page_size) {
			res = samd_issue_nvmctrl_command(bank->target, writecmd);
		} else {
