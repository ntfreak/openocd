...
static int avr_jtagprg_readfuses(struct avr_common *avr,
	uint32_t *page_buf,
	uint32_t buf_size,
	uint32_t addr,
	uint32_t page_size)
{
	...
	avr_jtag_senddat(avr->jtag_info.tap, &page_buf[0], 0x3e00, AVR_JTAG_REG_ProgrammingCommand_Len);
	avr_jtag_senddat(avr->jtag_info.tap, &page_buf[1], 0x3200, AVR_JTAG_REG_ProgrammingCommand_Len);
	avr_jtag_senddat(avr->jtag_info.tap, &page_buf[2], 0x3600, AVR_JTAG_REG_ProgrammingCommand_Len);
	avr_jtag_senddat(avr->jtag_info.tap, &page_buf[3], 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
...
static const struct avrf_type *avrf_get_avr_info(struct flash_bank *bank, int *error, uint8_t *ver)
{
	struct target *target = bank->target;
	struct avr_common *avr = target->arch_info;
	...
	*error = ERROR_FLASH_OPERATION_FAILED;
	return NULL;
}
