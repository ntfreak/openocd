 ...
 */
	...
	{"atmega2564rfr2", 0xa803, 256, 1024, 8, 1024, 3},
...
static int avrf_common_erase(struct flash_bank *bank, int first, int last, uint32_t enter_cmd)
{
	...
	LOG_DEBUG("%s %d-%d", __func__, first, last);
...
{
	return avrf_common_write(bank, buffer, offset, count, 0, 0x2310);
}
...
{
	return avr_common_flash_read(bank, buffer, offset, count, 0, 0x2302);
}
...
static int avr_eeprom_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
	struct avr_common *avr = target->arch_info;
	int status;
	uint32_t page_size, address, i, poll_value;
	...
	avr_jtag_sendinstr(avr->jtag_info.tap, NULL, AVR_JTAG_INS_PROG_COMMANDS);
	avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x2383, AVR_JTAG_REG_ProgrammingCommand_Len);
	for ( ; first <= last; first++) {
		address = first * page_size;
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x0700 | ((address >> 8) & 0xff), AVR_JTAG_REG_ProgrammingCommand_Len);
		for (i = 0; i < page_size; i++) {
			avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x0300 | ((address | i) & 0xff), AVR_JTAG_REG_ProgrammingCommand_Len);
			avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
			avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x7700, AVR_JTAG_REG_ProgrammingCommand_Len);
			avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
		}
...
static int avr_user_signature_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	avr_jtag_sendinstr(avr->jtag_info.tap, NULL, AVR_JTAG_INS_PROG_COMMANDS);
	avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x2384, AVR_JTAG_REG_ProgrammingCommand_Len);
	for ( ; first <= last; first++) {
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x0700 | (first + 1) / 2, AVR_JTAG_REG_ProgrammingCommand_Len);
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x0300 | ((first + 1) % 2) * 0x80 , AVR_JTAG_REG_ProgrammingCommand_Len);
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x7700, AVR_JTAG_REG_ProgrammingCommand_Len);
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
		avr_jtag_senddat(avr->jtag_info.tap, NULL, 0x3700, AVR_JTAG_REG_ProgrammingCommand_Len);
...
{
	return avrf_common_write(bank, buffer, offset, count, 0x100, 0x2312);
}

static int avr_user_signature_read(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count)
{
	return avr_common_flash_read(bank, buffer, offset, count, 0x100, 0x2308);
}
