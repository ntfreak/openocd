 ...
 * register count (ARMV7M_NUM_CORE_REGS for Cortex-M) since if (for Cortex-M) we
 * append BASEPRI or the VFP information to the returned register list we get a
 * "Remote 'g' packet reply is too long (expected 68 bytes, got XXX bytes)"
 * error from GDB. We need to ascertain the best solution to returning
 * per-thread BASEPRI and VFP information to GDB.
 *
 ...
 *
 * TODO: Extend openocd generic RTOS support to allow thread-specific FPU
 * register state to be returned. */
...
static int eCos_update_threads(struct rtos *rtos)
{
	...
	do {
		...
		rtos->thread_details[tasks_found].threadid = (threadid_t)thread_id;
