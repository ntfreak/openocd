...
int zephyr_get_arc_state(struct rtos *rtos, int64_t *addr,
			 ...
			 struct rtos_reg **reg_list, int *num_regs)
{
	...
	for (size_t i = 0; i < sizeof(arc_cpu_saved) / sizeof(struct stack_register_offset); i++) {
	...
	buf_cpy((*reg_list)[blink_offset].value,
		(*reg_list)[pc_offset].value, sizeof(uint32_t) * 8);
	...
	buf_cpy(&stack_top, (*reg_list)[sp_offset].value, sizeof(uint8_t) * 8);

	return retval;
}

/* ARM Cortex-M-specific implementation */
int zephyr_get_arm_state(struct rtos *rtos, int64_t *addr,
			 struct zephyr_params *params,
			 struct rtos_reg *callee_saved_reg_list,
			 struct rtos_reg **reg_list, int *num_regs)
{
...
int (*zephyr_cpu_get_state)(struct rtos *rtos, int64_t *addr,
			struct zephyr_params *params,
			struct rtos_reg *callee_saved_reg_list,
			struct rtos_reg **reg_list, int *num_regs);
...
static int zephyr_create(struct target *target)
{
	...
	for (struct zephyr_params *p = zephyr_params_list; p->target_name; p++) {
		if (!strcmp(p->target_name, name)) {
			if (!strcmp(name, "arcv2"))
				zephyr_cpu_get_state = &zephyr_get_arc_state;
			else if (!strcmp(name, "cortex_m") || !strcmp(name, "hla_target"))
				zephyr_cpu_get_state = &zephyr_get_arm_state;
...
static void zephyr_array_init(struct array *array)
{
	array->ptr = NULL;
	array->elements = 0;
}

static void zephyr_array_free(struct array *array)
{
	free(array->ptr);
	zephyr_array_init(array);
}

static void *zephyr_array_append(struct array *array, size_t size)
{
...
static void *zephyr_array_detach_ptr(struct array *array)
{
	void *ptr = array->ptr;
		...
		struct rtos_reg **reg_list, int *num_regs)
{
	...
	addr = thread_id + params->offsets[OFFSET_T_STACK_POINTER]
		 ...
		 retval = zephyr_get_arc_state(rtos, &addr, params, callee_saved_reg_list, reg_list, num_regs);
