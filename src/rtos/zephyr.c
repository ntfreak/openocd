		...
		struct rtos_reg **reg_list, int *num_regs)
{
	...
	if (!strcmp(params->target_name, "arcv2")) {
	...
	} else if (!strcmp(params->target_name, "cortex_m") || !strcmp(params->target_name, "hla_target")) {
		...
		LOG_ERROR("Error: Unsupported target: should be cortex_m or hla_target or arcv2.");
		return ERROR_FAIL;
	}
