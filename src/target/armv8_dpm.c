	...
	uint64_t *value)
{
	...
	target_el = 0;
	highest_el = target_el;		/* set default to run with current excetion level */
	scr_value = ns_requested;	/* set default not to restore the scr register */
		...
		if (ns_requested != MSRMRS_NOOPTION) {	/* user specified a specific security setting */
			if (ns_requested == MSRMRS_SECURE && highest_mode != ARMV8_64_EL3H) {
				...
				retval = dpm->instr_read_data_r0_64(dpm,
					...
					&scr_value);
				if (retval != ERROR_OK) {
					scr_value = ns_requested; /* no need to restore ns bit */
					goto fail;
				}
