...
/* should be const as it's never modified, but regs[i].value is not const */
static uint8_t regs_value[MAX_REG_SIZE / 8];

static int mem_ap_get_gdb_reg_list(struct target *target, struct reg **reg_list[],
				int *reg_list_size, enum target_register_class reg_class)
{
	struct mem_ap *mem_ap = target->arch_info;
	struct reg *regs = mem_ap->regs;
	int nregs = NUM_REGS;

	if (!regs) {
		regs = calloc(nregs, sizeof(struct reg));
		if (!regs) {
			LOG_ERROR("Out of memory");
			return ERROR_FAIL;
		}
	...
	*reg_list = calloc(nregs, sizeof(struct reg *));
	if (!*reg_list) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
