				...
				int *reg_list_size, enum target_register_class reg_class)
{
	struct reg *regs;
	uint8_t *value;
	unsigned int nregs = NUM_REGS;
	int len = (nregs * sizeof(struct reg *)) +	/* reg_list[] */
			  (nregs * sizeof(struct reg)) +	/* *reg_list[i] */
              (MAX_REG_SIZE / 8);				/* *reg_list[0]->value */

	*reg_list = calloc(1, len);
	if (!*reg_list) {
		LOG_ERROR("Out of memory");
		return ERROR_FAIL;
	}
	...
	*reg_list_size = nregs;
	...
	regs = (struct reg *)((uint8_t *)(*reg_list) + nregs * sizeof(struct reg *));
	value = (uint8_t *)regs + nregs * sizeof(struct reg);
