	...
	int num, enum arm_mode mode)
{
	...
	if (r->size <= 8) {
		...
		struct reg *r32;
		r32 = &armv7m->arm.core_cache->reg_list[ARMV7M_PMSK_BPRI_FLTMSK_CTRL];
		if (!r32->valid) {
			retval = armv7m_read_core_reg(target, r32, ARMV7M_PMSK_BPRI_FLTMSK_CTRL, mode);
			if (retval != ERROR_OK)
				return retval;
		}
	...
	int num, enum arm_mode mode, uint8_t *value)
{
	...
	if (r->size <= 8) {
		...
		buf_cpy(value, r32->value + (armv7m_core_reg->num - ARMV7M_PRIMASK),
				r->size);
		r32->dirty = true;
