...
static int autodetect_image_type(struct image *image, const char *url)
{
	...
	if (strncmp((char *)buffer, ELFMAG, SELFMAG) == 0) {
		assert(EI_CLASS < 9);
		switch (((char *) buffer)[EI_CLASS]) {
		case ELFCLASS32:
			LOG_DEBUG("ELF32 image detected.");
			image->type = IMAGE_ELF32;
			break;

		case ELFCLASS64:
			LOG_DEBUG("ELF64 image detected.");
			image->type = IMAGE_ELF64;
			break;

		default:
			LOG_ERROR("invalid ELF file, only 32/64bits files are supported");
...
static int identify_image_type(struct image *image, const char *type_string, const char *url)
{
	if (type_string) {
		if (!strcmp(type_string, "bin"))
			...
			image->type = IMAGE_IHEX;
		else if (!strcmp(type_string, "elf32"))
			image->type = IMAGE_ELF32;
		else if (!strcmp(type_string, "elf64"))
			image->type = IMAGE_ELF64;
...
static int image_elf64_read_headers(struct image *image)
{
	...
	elf->header = malloc(sizeof(Elf64_Ehdr));
	...
	retval = fileio_read(elf->fileio, sizeof(Elf64_Ehdr), (uint8_t *)elf->header, &read_bytes);
	if (retval != ERROR_OK) {
		LOG_ERROR("cannot read ELF file header, read failed");
		return ERROR_FILEIO_OPERATION_FAILED;
	}
	...
	retval = fileio_seek(elf->fileio, field64(elf, elf->header->e_phoff));
	if (retval != ERROR_OK) {
		LOG_ERROR("cannot seek to ELF program header table, read failed");
		return retval;
	}
	...
	image->start_address = field64(elf, elf->header->e_entry);
	...
	size_t *size_read)
{
	...
	*size_read = 0;
	...
	if (offset < field64(elf, segment->p_filesz)) {
		...
		LOG_DEBUG("read elf: size = 0x%zu at 0x%" TARGET_PRIxADDR "", read_size,
...
int image_open(struct image *image, const char *url, const char *type_string)
{
	...
	} else if (image->type == IMAGE_ELF32) {
		struct image_elf32 *image_elf32;

		image_elf32 = image->type_private = malloc(sizeof(struct image_elf32));

		retval = fileio_open(&image_elf32->fileio, url, FILEIO_READ, FILEIO_BINARY);
		if (retval != ERROR_OK)
			return retval;

		retval = image_elf32_read_headers(image);
		if (retval != ERROR_OK) {
			fileio_close(image_elf32->fileio);
			return retval;
		}
	} else if (image->type == IMAGE_ELF64) {
		struct image_elf64 *image_elf64;

		image_elf64 = image->type_private = malloc(sizeof(struct image_elf64));

		retval = fileio_open(&image_elf64->fileio, url, FILEIO_READ, FILEIO_BINARY);
		if (retval != ERROR_OK)
			return retval;

		retval = image_elf64_read_headers(image);
		if (retval != ERROR_OK) {
			fileio_close(image_elf64->fileio);
			return retval;
		}
...
void image_close(struct image *image)
{
	if (image->type == IMAGE_BINARY) {
		...
		struct image_elf64 *image_elf = image->type_private;
