...
int cortex_m_examine(struct target *target)
{
	...
	if (!target_was_examined(target)) {
		...
		armv7m->arm.arch = core_info->arch;

		LOG_DEBUG("%s r%" PRId8 "p%" PRId8 " processor detected",
				core_info->name, (uint8_t)((cpuid >> 20) & 0xf), (uint8_t)((cpuid >> 0) & 0xf));
		...
		if (core_info->flags & CORTEX_M_F_HAS_FPV4) {
			target_read_u32(target, MVFR0, &mvfr0);
			target_read_u32(target, MVFR1, &mvfr1);

			/* test for floating point feature on Cortex-M4 */
			if ((mvfr0 == MVFR0_DEFAULT_M4) && (mvfr1 == MVFR1_DEFAULT_M4)) {
				LOG_DEBUG("%s floating point feature FPv4_SP found", core_info->name);
				armv7m->fp_feature = FPV4_SP;
			}
		} else if (core_info->flags & CORTEX_M_F_HAS_FPV5) {
			target_read_u32(target, MVFR0, &mvfr0);
			target_read_u32(target, MVFR1, &mvfr1);

			/* test for floating point features on Cortex-M7 */
			if ((mvfr0 == MVFR0_DEFAULT_M7_SP) && (mvfr1 == MVFR1_DEFAULT_M7_SP)) {
				LOG_DEBUG("%s floating point feature FPv5_SP found", core_info->name);
				armv7m->fp_feature = FPV5_SP;
			} else if ((mvfr0 == MVFR0_DEFAULT_M7_DP) && (mvfr1 == MVFR1_DEFAULT_M7_DP)) {
				LOG_DEBUG("%s floating point feature FPv5_DP found", core_info->name);
		...
		if (!armv7m->stlink) {
			if (core_info->flags & CORTEX_M_F_TAR_AUTOINCR_BLOCK_4K)
