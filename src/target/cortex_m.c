 ...
 *  to preserve S_RESET_ST state in the case of a reset event was detected.
 */
...
static int cortex_m_poll(struct target *target)
{
	...
	if (target->state == TARGET_UNKNOWN) {
		 ...
		 * Don't use the cumulated sticky state (dcb_dhcsr_cumulated_sticky)
		 * as we're testing if the target is running now, not if it has been
		 * running (possibly in the past). Instructions are typically processed
		 * much faster than OpenOCD can read DHCSR so S_RETIRE_ST reads always
		 * as 1 if the target runs.
		 */
