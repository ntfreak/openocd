...
static int adiv6_dap_get_debugbase(struct adiv5_ap *ap, target_addr_t *dbgbase, uint32_t *apid);
static int adiv6_dap_info_command(struct command_invocation *cmd, struct adiv5_ap *ap);
		...
		uint32_t value)
{
	...
	return dap_queue_ap_write(ap, MEM_AP_REG_BD0(ap->dap) | (address & 0xC), value);
}
...
int mem_ap_init(struct adiv5_ap *ap)
{
	...
	ap->cfg_reg = cfg;
	if (is_64bit_ap(ap)) {
		retval = dap_queue_ap_write(ap, MEM_AP_REG_TAR64(dap), 0);
		if (retval == ERROR_OK)
			retval = dap_run(dap);
		if (retval != ERROR_OK) {
			ap->tar_valid = false;
...
int dap_find_ap(struct adiv5_dap *dap, enum ap_type type_to_find, struct adiv5_ap **ap_out)
{
	...
	for (ap_num = 0; ap_num <= DP_APSEL_MAX; ap_num++) {
		...
		int retval = dap_queue_ap_read(dap_ap(dap, ap_num), AP_REG_IDR(dap), &id_val);
		if (retval != ERROR_OK)
			return retval;
		...
		if ((retval == ERROR_OK) &&                  /* Register read success */
			...
			(type_to_find == AP_TYPE_APB_AP ? (id_val & IDR_TYPE) == AP_TYPE_APB4_AP : 0))
		   ) {      /* type matches*/
			...
			target_addr_t dbgbase, uint8_t type, target_addr_t *addr, int32_t *idx)
{
	...
	struct adiv5_dap *dap = ap->dap;
	if (dap->adi_version == 6)
		return adiv6_dap_lookup_cs_component(ap, dbgbase, type, addr, idx);
		...
		struct adiv5_ap *ap)
{
	...
	/* ap->dap->ap[0] is reserved for the DP ROM Table */
	if ((dap->adi_version == 6) && (ap == ap->dap->ap))
	...
	case IDR_JEP106_ARM | AP_TYPE_APB_AP:
		...
		break;
	case IDR_JEP106_ARM | AP_TYPE_APB4_AP:
		command_print(cmd, "\tType is MEM-AP APB4");
		break;
...
COMMAND_HANDLER(dap_baseaddr_command)
{
	...
	if (dap->adi_version == 6) {
		if (ap->cfg_reg != ADI_BAD_CFG)
			command_print(CMD, "0x%016" PRIx64, dap_ap(dap, apsel)->base_addr);
		else {
...
COMMAND_HANDLER(dap_apid_command)
{
	...
	retval = dap_queue_ap_read(dap_ap(dap, apsel), AP_REG_IDR(dap), &apid);
	if (retval != ERROR_OK)
		return retval;
	retval = dap_run(dap);
	if (retval != ERROR_OK)
			...
			target_addr_t dbgbase, uint8_t type, target_addr_t *addr, int32_t *idx)
{
	...
	*addr = 0;
	retval = mem_ap_read_atomic_u32(ap, dbgbase | AP_REG_DEVID, &devid_reg);
	if (retval == ERROR_OK)
		retval = mem_ap_read_atomic_u32(ap, dbgbase | AP_REG_CIDR1, &cidr1);
	if (retval != ERROR_OK)
		return retval;
	...
	do {
		if (rom_entry_64bit) {
			...
			if (is_64bit_ap(ap)) {
				/* 64-bit addressable APB with 32-bit ROM entry, sign extend */
				romentry = (target_addr_t) ((int32_t) romentry_lower);
			} else {
				/* 32-bit addressable APB with 32-bit ROM entry, no sign extension */
				romentry = (target_addr_t) ((uint32_t) romentry_lower);
			}
		}
		...
		if (romentry & 0x1) {
			...
			if (((c_cid1 >> 4) & 0x0f) == 9) {
				uint32_t devarch;
				retval = mem_ap_read_atomic_u32(ap, component_base | 0xFBC, &devarch);
				if (retval != ERROR_OK)
					return retval;
...
static int adiv6_dap_rom_display_table(struct command_invocation *cmd,
				struct adiv5_ap *ap, target_addr_t base_addr, int depth)
{
	int retval;
	...
	if (retval == ERROR_OK)
		retval = mem_ap_read_atomic_u32(ap, base_addr | AP_REG_CIDR1, &cidr1);
	if (retval != ERROR_OK)
		return retval;
	...
	for (entry_offset = 0; entry_offset < max_entry_offset; entry_offset += 4) {
		...
		if (rom_entry_64bit) {
			retval = mem_ap_read_atomic_u32(ap, base_addr | entry_offset, &romentry_upper);
			if (retval != ERROR_OK)
				return retval;
			retval = mem_ap_read_atomic_u32(ap, base_addr | (entry_offset + 4), &romentry_lower);
			if (retval != ERROR_OK)
				return retval;
			romentry = (((target_addr_t) romentry_upper) << 32) | romentry_lower;
			...
			retval = mem_ap_read_atomic_u32(ap, base_addr | entry_offset, &romentry_lower);
			if (retval != ERROR_OK)
				return retval;
			if (is_64bit_ap(ap)) {
				/* 64-bit addressable APB with 32-bit ROM entry, sign extend */
		...
		if (romentry & 0x01) {
			/* Recurse */
			retval = adiv6_dap_rom_display(cmd, ap, base_addr + (romentry & 0xFFFFFFFFFFFFF000ull),
				depth + 1);
			if (retval != ERROR_OK)
				return retval;
		} else if (romentry != 0) {
				...
				struct adiv5_ap *ap, target_addr_t dbgbase, int depth)
{
	...
	if (class == 1) { /* ROM Table */
		...
		retval = mem_ap_read_atomic_u32(ap, base_addr | 0xFBC, &devarch);
		if (retval != ERROR_OK)
			return retval;
...
static int adiv6_get_dp_rom_info(struct command_invocation *cmd, struct adiv5_ap *ap, uint32_t *cidr1_reg,
	uint32_t *cfg_reg, uint32_t *devid_reg, uint32_t *devarch_reg, uint32_t *id_reg)
{
	...
	if (id_reg != NULL)
		retval = dap_queue_ap_read(ap, AP_REG_IDR(ap->dap), id_reg);
	if (retval == ERROR_OK)
	...
	if (retval == ERROR_OK && cfg_reg != NULL)
		retval = dap_queue_ap_read(ap, MEM_AP_REG_CFG(ap->dap), cfg_reg);
		...
		struct adiv5_ap *ap)
{
	...
	struct adiv5_ap rom_ap = ap->dap->ap[0]; /* ap[0] is reserved for the DP ROM Table */
	...
	for (entry_offset = 0; entry_offset < max_entry_offset; entry_offset += 4) {
		...
		if (rom_entry_64bit) {
			...
			if (is_64bit_ap(ap)) {
				...
				/* 32-bit addressable APB with 32-bit ROM entry, no sign extension */
				entry = (target_addr_t) ((uint32_t) entry_lower);
				command_print(cmd, "ROM entry[0x%x]: 0x%8.8" PRIx32, entry_offset, entry_lower);
			}
		}
		...
		if ((entry & 3) == 3) {
			...
			rom_ap.base_addr = baseptr;
			if (retval != ERROR_OK) {
				command_print(cmd, "\t\tROM Table looks wrong; will try using target "
					"mem_ap for this entry");
				if (ap->cfg_reg == ADI_BAD_CFG) {
					command_print(CMD, "\t\tAccess Port %d not determined to be a mem_ap",
							ap->ap_num);
					continue;
				}

				retval = dap_queue_ap_read(ap, AP_REG_IDR(ap->dap), &id_val);
				if (retval == ERROR_OK)
					retval = dap_run(ap->dap);
				if (((id_val & IDR_TYPE) != AP_TYPE_APB_AP) && ((id_val & IDR_TYPE) !=
					AP_TYPE_APB4_AP)) {
					command_print(CMD, "\t\tAccess Port %d not determined to be an APB",
						ap->ap_num);
					continue;
				}
