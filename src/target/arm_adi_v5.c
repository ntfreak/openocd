		...
		uint8_t *buffer, uint32_t size, uint32_t count, uint32_t address)
{
	...
	if (ap->dap->ops->ap_mem_read) {
		retval = ap->dap->ops->ap_mem_read(ap, buffer, size, count, address, true);
		...
		const uint8_t *buffer, uint32_t size, uint32_t count, uint32_t address)
{
	...
	if (ap->dap->ops->ap_mem_write) {
		retval = ap->dap->ops->ap_mem_write(ap, buffer, size, count, address, true);
		if (retval != ERROR_OP_NOT_SUPPORTED)
			return retval;
	}
		...
		uint8_t *buffer, uint32_t size, uint32_t count, uint32_t address)
{
	...
	if (ap->dap->ops->ap_mem_read) {
		retval = ap->dap->ops->ap_mem_read(ap, buffer, size, count, address, false);
		if (retval != ERROR_OP_NOT_SUPPORTED)
			return retval;
	}
		...
		const uint8_t *buffer, uint32_t size, uint32_t count, uint32_t address)
{
	...
	if (ap->dap->ops->ap_mem_write) {
		retval = ap->dap->ops->ap_mem_write(ap, buffer, size, count, address, false);
		if (retval != ERROR_OP_NOT_SUPPORTED)
			return retval;
	}
