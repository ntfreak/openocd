		...
		int argc, Jim_Obj * const *argv)
{
	...
	case 64:
		break;
	default:
		Jim_SetResultString(interp, "invalid width param, must be 8, 16, 32 or 64", -1);
		return JIM_ERR;
	}
	...
	if ((width == 1) ||
		...
		char buf[128];
		Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
		snprintf(buf, sizeof(buf), "read_memory: " TARGET_ADDR_FMT " is not aligned for %u byte reads",
			addr, width);
		Jim_AppendStrings(interp, Jim_GetResult(interp), buf, NULL);
		return JIM_ERR;
	}
...
static int jim_read_memory(Jim_Interp *interp, int argc, Jim_Obj * const *argv)
{
	...
	return target_jim_read_memory(interp, target, argc, argv);
}
		...
		int argc, Jim_Obj *const *argv)
{
	...
	if ((width == 1) ||
		...
		char buf[128];
		Jim_SetResult(interp, Jim_NewEmptyStringObj(interp));
		snprintf(buf, sizeof(buf), "write_memory: " TARGET_ADDR_FMT " is not aligned for %u byte writes",
			addr, width);
		Jim_AppendStrings(interp, Jim_GetResult(interp), buf, NULL);
		return JIM_ERR;
	}
...
static int jim_write_memory(Jim_Interp *interp, int argc, Jim_Obj * const *argv)
{
	struct command_context *context;
	...
	return target_jim_write_memory(interp, target, argc, argv);
}
...
static int target_jim_get_reg(Jim_Interp *interp, struct target *target,
		int argc, Jim_Obj * const *argv)
{
	if (argc != 2) {
	...
	for (int i = 0; i < length; i++) {
		const Jim_Obj *elem = Jim_ListGetIndex(interp, argv[1], i);
...
static int target_jim_set_reg(Jim_Interp *interp, struct target *target,
		int argc, Jim_Obj * const *argv)
{
	if (argc != 2) {
...
static int jim_get_reg(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	return target_jim_get_reg(interp, target, argc, argv);
}
...
static int jim_set_reg(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	return target_jim_set_reg(interp, target, argc, argv);
}
...
static const struct command_registration target_instance_command_handlers[] = {
	...
	},
	{
		...
		.help = "Load Tcl list of 8/16/32/64 bit numbers from target memory",
