...
static int jim_arm_tpiu_swo_enable(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	if (transport_is_hla() && obj->spot.ap_num != 0) {
		LOG_ERROR("Invalid access port. Only AP#0 allowed with hla transport");
		return JIM_ERR;
	}

	if (!obj->traceclkin_freq) {
		LOG_ERROR("Invalid trace clock-in frequency");
		return JIM_ERR;
	}
	...
	if (obj->pin_protocol == TPIU_SPPR_PROTOCOL_MANCHESTER || obj->pin_protocol == TPIU_SPPR_PROTOCOL_UART) {
		if (!obj->swo_pin_freq) {
			LOG_ERROR("Invalid SWO pin frequency");
			return JIM_ERR;
		}

		unsigned int presc = (obj->traceclkin_freq + obj->swo_pin_freq / 2) / obj->swo_pin_freq;
		if (presc > TPIU_ACPR_MAX_PRESCALER)
			presc = TPIU_ACPR_MAX_PRESCALER;
		prescaler = presc;
	}

	retval = wrap_read_u32(target, tpiu_ap, obj->spot.address + TPIU_DEVID_OFFSET, &value);
	if (retval != ERROR_OK)
		return JIM_ERR;
	...
	if (!value) {
		...
		LOG_ERROR("Protocol %s not supported", p->name);
		return JIM_ERR;
	}

	if (obj->pin_protocol == TPIU_SPPR_PROTOCOL_SYNC) {
		retval = wrap_read_u32(target, tpiu_ap, obj->spot.address + TPIU_SSPSR_OFFSET, &value);
	...
	if (obj->out_filename && strcmp(obj->out_filename, "external") && obj->out_filename[0]) {
		...
		retval = adapter_config_trace(true, obj->pin_protocol, obj->port_width,
			&obj->swo_pin_freq, obj->traceclkin_freq, &prescaler);
		if (retval != ERROR_OK) {
	...
	retval = wrap_write_u32(target, tpiu_ap, obj->spot.address + TPIU_CSPSR_OFFSET, BIT(obj->port_width - 1));
	if (retval != ERROR_OK)
		goto error_exit;

	retval = wrap_write_u32(target, tpiu_ap, obj->spot.address + TPIU_ACPR_OFFSET, prescaler - 1);
	if (retval != ERROR_OK)
		goto error_exit;

	retval = wrap_write_u32(target, tpiu_ap, obj->spot.address + TPIU_SPPR_OFFSET, obj->pin_protocol);
	if (retval != ERROR_OK)
		goto error_exit;

	retval = wrap_read_u32(target, tpiu_ap, obj->spot.address + TPIU_FFCR_OFFSET, &value);
	if (retval != ERROR_OK)
		goto error_exit;
	if (obj->en_formatter)
		value |= BIT(1);
		...
		value &= ~BIT(1);
	retval = wrap_write_u32(target, tpiu_ap, obj->spot.address + TPIU_FFCR_OFFSET, value);
	if (retval != ERROR_OK)
		goto error_exit;
...
static int jim_arm_tpiu_swo_disable(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	/* FIXME: which target should be called?
...
static int jim_arm_tpiu_swo_create(Jim_Interp *interp, int argc, Jim_Obj *const *argv)
{
	...
	obj->spot.address = TPIU_SWO_DEFAULT_BASE;
	obj->port_width = 1;
