...
static int arc_get_register(struct reg *reg)
{
	...
	if (desc->is_core) {
		if (desc->arch_num == 61 || desc->arch_num == 62) {
...
static int arc_set_register(struct reg *reg, uint8_t *buf)
{
	...
	* is in halt mode. Async writes are not supported yet. */
	...
	if (desc->is_core && (desc->arch_num == 61 ||
			desc->arch_num == 62)) {
...
static void arc_init_reg(struct target *target, struct reg *reg,
...
static int arc_build_reg_cache(struct target *target)
{
	...
	list_for_each_entry(reg_desc, &arc->core_reg_descriptions, list) {
		arc_init_reg(target, &reg_list[i], reg_desc, i);
...
static int arc_build_bcr_reg_cache(struct target *target)
{
	...
	list_for_each_entry(reg_desc, &arc->bcr_reg_descriptions, list) {
		arc_init_reg(target, &reg_list[i], reg_desc, gdb_regnum);
		...
		const char *field_name, uint32_t *value_ptr)
{
	...
	uint32_t *register_value = calloc(1, sizeof(uint32_t));
	if (!register_value) {
		LOG_ERROR("Unable to allocate memory");
		free(register_value);
		return ERROR_FAIL;
	...
	*register_value = target_buffer_get_u32(target, reg->value);
	memcpy(buffer, register_value, sizeof(uint32_t));
	*value_ptr = buf_get_u32(buffer, field->bitfield->start,
...
static void arc_deinit_target(struct target *target)
{
	...
	list_for_each_entry_safe_reverse(type, n, &arc->reg_data_types, list) {
		if (type->data_type.type_class == REG_TYPE_CLASS_STRUCT) {
			int i = 0;
			while (type->data_type.reg_type_struct->fields[i].name) {
					free((char *)type->data_type.reg_type_struct->fields[i].name);
					i++;
			}
			free((char *)type->data_type.id);
			free(type->data_type.reg_type_struct->fields->bitfield);
			free(type->data_type.reg_type_struct->fields);
			free(type->data_type.reg_type_struct);
			free(type);
		}	else if (type->data_type.type_class == REG_TYPE_CLASS_FLAGS) {
			int i = 0;
			while (type->data_type.reg_type_flags->fields[i].name) {
					free((char *)type->data_type.reg_type_flags->fields[i].name);
					i++;
			}
			free((char *)type->data_type.id);
			free(type->data_type.reg_type_flags->fields->bitfield);
			free(type->data_type.reg_type_flags->fields);
			free(type->data_type.reg_type_flags);
			free(type);
		}
	}
