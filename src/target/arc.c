...
static int arc_build_reg_cache(struct target *target)
{
	...
	if (!list_empty(&arc->aux_reg_descriptions)) {
		list_for_each_entry(reg_desc, &arc->aux_reg_descriptions, list) {
			...
			if (arc->pc_index_in_cache == ULONG_MAX && !strcmp("pc", reg_desc->name))
				arc->pc_index_in_cache = i;
			else if (arc->debug_index_in_cache == ULONG_MAX
					&& !strcmp("debug", reg_desc->name))
...
static int arc_regs_get_core_reg(struct reg *reg)
{
	...
	if (arc_reg->desc->is_core) {
		...
		CHECK_RETVAL(arc_jtag_read_core_reg_one(&arc->jtag_info, arc_reg->desc->arch_num,
			&arc_reg->value));
	} else {
		CHECK_RETVAL(arc_jtag_read_aux_reg_one(&arc->jtag_info, arc_reg->desc->arch_num,
			&arc_reg->value));
	}
