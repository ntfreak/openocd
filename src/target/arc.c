...
static int arc_regs_get_core_reg(struct reg *reg)
{
	...
	struct arc_register *arc_reg = reg->arch_info;
	struct target *target = arc_reg->target;
	struct arc_common *arc = target_to_arc(target);
	struct arc_reg_desc *desc = arc_reg->desc;

	if (reg->valid) {
		LOG_DEBUG("Get register (cached) gdb_num=%" PRIu32 ", name=%s, value=0x%" PRIx32,
				reg->number, desc->name, arc_reg->value);
		return ERROR_OK;
	}
	...
	target_buffer_set_u32(target, reg->value, arc_reg->value);
...
static void arc_init_reg(struct target *target, struct reg *reg,
			struct arc_register *arc_reg, struct arc_reg_desc *reg_desc,
			unsigned long number)
{
	...
	/* Initialize struct arc_register */
...
static int arc_build_reg_cache(struct target *target)
{
	...
	struct arc_register *reg_arch_info = calloc(num_regs, sizeof(struct arc_register));
		...
		uint32_t *value_ptr)
{
	...
	const struct arc_register * const arc_r = reg->arch_info;
...
static void arc_free_reg_cache(struct target *target)
{
	struct arc_common *arc = target_to_arc(target);
