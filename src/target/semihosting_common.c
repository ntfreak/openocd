	...
	void *post_result)
{
	...
	semihosting->tcp_config = SEMIHOSTING_TCP_CFG_NONE;
...
{
	if (semihosting->tcp_config != SEMIHOSTING_TCP_CFG_NONE) {
		/* WRITE command is redirected to TCP only when the config is set to ALL */
		if (semihosting->op == SEMIHOSTING_SYS_WRITE
					&& semihosting->tcp_config != SEMIHOSTING_TCP_CFG_ALL)
			goto default_write;
		...
		if (semihosting->tcp_connection) {
			...
			/* else (the fd is not a ':tt' file), goto default_write */
		}
		/* else: no connected client, goto default_write */
	}
...
{
	if (semihosting->tcp_config != SEMIHOSTING_TCP_CFG_NONE) {
		/* READ command is redirected to TCP only when the config is set to ALL */
		if (semihosting->op == SEMIHOSTING_SYS_READ
					&& semihosting->tcp_config != SEMIHOSTING_TCP_CFG_ALL)
			goto default_read;
		...
		if (semihosting->tcp_connection) {
			if (fd == semihosting->stdin_fd)
				/* TODO */
				return -1;
			/* else (the fd is not a ':tt' file), goto default_read */
		}
		/* else: no connected client, goto default_read */
	}
...
{
	int c;
	return semihosting_read(semihosting, fd, &c, 1) > 0 ? c : EOF;
}
...
static __COMMAND_HANDLER(handle_common_semihosting_over_tcp_command)
{
	struct target *target = get_current_target(CMD_CTX);
	...
	enum semihosting_tcp_config cfg;
	const char *port;

	if (strcmp(CMD_ARGV[0], "disable") == 0) {
		cfg = SEMIHOSTING_TCP_CFG_NONE;
		if (CMD_ARGC > 1)
		...
		if (strcmp(CMD_ARGV[1], "debug") == 0)
			cfg = SEMIHOSTING_TCP_CFG_DEBUG;
		else if (strcmp(CMD_ARGV[1], "all") == 0)
			cfg = SEMIHOSTING_TCP_CFG_ALL;
		else
			return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (cfg > SEMIHOSTING_TCP_CFG_NONE) {
		...
		int ret = add_service("semihosting_redirect", port, 1,
				semihosting_tcp_new_cnx, semihosting_tcp_cnx_input,
				semihosting_tcp_cnx_closed, service);
		if (ret != ERROR_OK) {
...
const struct command_registration semihosting_common_handlers[] = {
	...
	{
		.name = "semihosting_over_tcp",
		.handler = handle_common_semihosting_over_tcp_command,
		.mode = COMMAND_EXEC,
		.usage = "(disable | <port> (debug | all))",
		.help = "redirect semihosting to through TCP",
	},
	{
