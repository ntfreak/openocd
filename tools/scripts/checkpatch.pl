...
# OpenOCD specific: load list of CamelCase names
my $camelcase_names;
if (show_type("CAMELCASE")) {
	my $permitted_camelcase_file = "$root/tools/scripts/camelcase.txt";
	read_words(\$camelcase_names, $permitted_camelcase_file)
		or warn "No camelcase to ignore - file '$permitted_camelcase_file': $!\n";
}
...
# Check for various typo / spelling mistakes
		if (defined($misspellings) &&
		    # OpenOCD specific: don't check spelling on spelling_file
...
#CamelCase
			# OpenOCD specific: use list of CamelCase names
			if (defined($camelcase_names) &&
			    $var =~ /\b($camelcase_names)\b/) {
				next;
			}
